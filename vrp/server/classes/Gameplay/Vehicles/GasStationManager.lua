...
function GasStationManager:confirmTransaction(vehicle, fuel, station, opticalFuel, price)
	...
	if station then
		if instanceof(vehicle, PermanentVehicle, true) or instanceof(vehicle, GroupVehicle, true) or instanceof(vehicle, FactionVehicle, true) or instanceof(vehicle, CompanyVehicle, true) then
			...
			if fuel == 0 then
				client:sendError(_("Your vehicle already has a full tank!", client))
				return
			end
			...
			if station:isUserFuelStation() then
				...
				if faction and (faction:isEvilFaction() or client:isFactionDuty()) and instanceof(vehicle, FactionVehicle, true) then
					if faction:getMoney() >= price then
						client:sendInfo(_("The vehicle has been refueled!", client))
						faction:transferMoney(self.m_BankAccountServer, price, "Tanken", "Vehicle", "Refill")
						faction:addLog(client, "Tanken", ("has refueled the vehicle %s (%s) for %s$!"):format(vehicle:getName(), vehicle:getPlateText(), price))
						vehicle:setFuel(vehicle:getFuel() + fuel)

						client:triggerEvent("gasStationReset")
					else
						client:sendError(_("There is not enough money in the Group's coffers!", client))
						return
					end
				elseif company and client:isCompanyDuty() and instanceof(vehicle, CompanyVehicle, true) then
					if company:getMoney() >= price then
						...
						client:sendInfo(_("The vehicle has been refueled!", client))
						company:addLog(client, "Tanken", ("has refueled the vehicle %s (%s) for %s$!"):format(vehicle:getName(), vehicle:getPlateText(), price))
						vehicle:setFuel(vehicle:getFuel() + fuel)

						client:triggerEvent("gasStationReset")
					else
						client:sendError(_("There is not enough money in the company's coffers!", client))
						return
					end
				elseif client:getMoney() >= price then
					...
					client:sendError(_("You don't have enough money with you!", client))
					return
				end
				...
				if station:getShop() then
					...
					client:sendInfo(_("%s thanks for your purchase!", client, station:getName()))
			...
			elseif station:isFactionFuelStation() then
				if not instanceof(vehicle, FactionVehicle, true) then client:sendWarning("This vehicle may not be fueled here!") return end
			...
			elseif station:isCompanyFuelStation() then
				if not instanceof(vehicle, CompanyVehicle, true) then client:sendWarning("This vehicle may not be fueled here!") return end
