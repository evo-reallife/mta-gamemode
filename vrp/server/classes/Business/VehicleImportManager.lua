...
function VehicleImportManager:startTransport(shopId, model, variant, reloadListForClient)
	if not client or not client:getCompany() or not client:isCompanyDuty() or client:getCompany():getId() ~= CompanyStaticId.EPT then 
		if client then client:sendError("You are not allowed to start the transport.") end
		return 
	end
	if not PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "company", "startVehicleImport") then
		client:sendError(_("You are not authorized to start a vehicle import!", client))
		return
	end
	...
	if not ShopManager.VehicleShopsMap[shopId] or not ShopManager.VehicleShopsMap[shopId].m_VehicleList[model] or not ShopManager.VehicleShopsMap[shopId].m_VehicleList[model][variant] then
		client:sendError("Invalid store vehicle. Please open the vehicle list again.")
		return
	end
	...
	if current >= max then
		client:sendError("The store does not require any additional vehicles.")
		return
	end

	CompanyManager:getSingleton():getFromId(CompanyStaticId.EPT)
			:sendShortMessage(("%s has started the transport for the vehicle '%s'."):format(getPlayerName(client), VehicleCategory:getSingleton():getModelName(model)))
	CompanyManager:getSingleton():getFromId(CompanyStaticId.EPT):addLog(client, "Import", ("has started the vehicle transport for the vehicle '%s'!"):format(VehicleCategory:getSingleton():getModelName(model)))
...
function VehicleImportManager:internalOnVehicleTransporterDamage(transporter, loss)
	...
	if transporter.controller and vehiclesDetached > 0 then
		transporter.controller:sendWarning("The impact of the transporter has thrown the vehicles away! Load them up again quickly or the transport will fail!")
	end
end
...
function VehicleImportManager:internalDestroyVehicle(veh, missionSuccess)
	...
	if not missionSuccess then
		CompanyManager:getSingleton():getFromId(CompanyStaticId.EPT)
		:sendShortMessage(("The delivery of vehicle '%s' was canceled."):format(veh:getName()))
	end
...
function VehicleImportManager:internalVehicleDeliveryColHit(col, dim)
	...
	if col == source.destinationCol then
		if source.controller and isElement(source.controller) then
			source.controller:sendInfo("Get out of the vehicle to complete delivery.")
		end
	end
end
...
function VehicleImportManager:internalOnVehicleExit(player, seat)
	...
	if veh.destinationCol and isElement(veh.destinationCol) and isElementWithinColShape(veh, veh.destinationCol) then
		if not player:getCompany() or player:getCompany():getId() ~= CompanyStaticId.EPT then 
			player:sendError(_("You cannot return the vehicle.", player))
			return false
		end
		if veh:getVelocity().length > 0.001 then 
			player:sendWarning("Drive more slowly to hand over the vehicle.")
			return false 
		end
		if veh:getEngineState() then 
			player:sendWarning("Switch off the engine to hand over the vehicle.")
			return false 
		end
		...
		eptInstance:sendShortMessage(("%s has delivered the vehicle '%s' (+%s)."):format(getPlayerName(player), veh:getName(), toMoneyString(moneyEarnedCompany)))
		eptInstance:addLog(player, "Import", ("has delivered the vehicle '%s' (+%s)!"):format(veh:getName(), toMoneyString(moneyEarnedCompany)))
		self:increaseShopStockByVehicle(veh.transportShopId, veh:getModel(), veh.transportShopVariant)

		player:giveCombinedReward("Vehicle delivered", {
			money = {
				mode = "give",
				bank = true,
		...
		self.m_BankAccountServer:transferMoney({eptInstance, true, true}, moneyEarnedCompany, "Vehicle delivered", "Action", "Vehicle Import")
...
function VehicleImportManager:internalOnVehicleDamage(vehicle, loss)
	...
	if controller then
		controller:sendError(("The vehicle '%s' is so broken that the dealership will no longer accept it!"):format(vehicle:getName()))
	end
