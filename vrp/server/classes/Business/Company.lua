...
function Company:__giveMoney(amount, reason, silent)
    StatisticsLogger:getSingleton():addMoneyLog("company", self, amount, reason or "Unknown")
...
function Company:removePlayer(playerId)
	...
	if player then
		...
		player:reloadBlips()
		player:sendShortMessage(_("You have been dismissed from your company!", player))
		self:sendShortMessage(_("%s has left your company!", player, player:getName()))
...
function Company:sendChatMessage(sourcePlayer,message)
	if not getElementData(sourcePlayer, "CompanyChatEnabled") then return sourcePlayer:sendError(_("You have deactivated the company chat!", sourcePlayer)) end
...
function Company:invitePlayer(player)
    client:sendShortMessage(("You have successfully invited %s to your company."):format(getPlayerName(player)))
    		...
    		function(hitElement)
    			if getElementType(hitElement) == "player" and not hitElement.vehicle then
    				...
    				if company and company == self then
                        ...
                        hitElement:sendError(_("You are not in this company!", hitElement))
                    end
    			end
    			cancelEvent()
    		end
...
function Company:respawnVehicles(player)
	...
	if not self:isRespawnPossible() and not isAdmin then
		return self:sendShortMessage("Vehicles can only be respawned every 15 minutes!")
	end
	...
	if player and caller then
		if instanceof(caller, Player) and instanceof(player, Player) then -- check if we can call methods from the Player-class
			if player.m_PhoneOn == false then
				player:sendError(_("Your phone is switched off!", player))
				return
			end
			...
			self:addLog(player, "Anrufe", ("has a telephone conversation with %s!"):format(caller:getName()))
	...
	addEventHandler("onElementClicked", self.m_Safe, function(button, state, player)
		if button == "left" and state == "down" then
			if player:getCompany() and player:getCompany() == self then
				...
				player:sendError(_("You are not in the right company!", player))
			end
		end
	end)
