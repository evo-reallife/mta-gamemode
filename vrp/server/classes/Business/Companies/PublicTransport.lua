...
function PublicTransport:start(player) -- on start duty
	...
	player:sendInfo(("The following car dealerships urgently need vehicles:%s"):format(shopsToSend))
end
...
function PublicTransport:onVehicleStartEnter(veh, player, seat)
	if seat > 0 and not veh:getOccupant(0) then
		if veh:getModel() ~= 437 and veh:getModel() ~= 409 and veh:getModel() ~= 578 then
			cancelEvent()
			player:sendError(_("No %s is present.", player, veh:getModel() == 487 and "pilot in the helicopter" or "driver in the taxi"))
		elseif veh:getModel() == 437 then
			cancelEvent()
			player:sendError(_("No driver is present in the bus.", player))
		end
	elseif veh:getModel() == 437 and not veh.Bus_OnDuty then
		cancelEvent()
		player:sendError(_("This bus is not on duty.", player))
	end
end
...
function PublicTransport:endTaxiDrive(customer)
    if self.m_TaxiCustomer[customer] then
        ...
        if price > customer:getBankMoney() then
            price = customer:getBankMoney()
        end
        ...
        customer:sendInfo(_("You have exited the taxi! The ride cost you %d$", customer, price))
        ...
        if priceForEPT > 0 then
            if not customer:getCompany() or customer:getCompany():getId() ~= CompanyStaticId.EPT then
                self.m_BankAccountServer:transferMoney({self, nil, true}, priceForEPT, ("Taxi ride from %s with %s"):format(driverName, customer:getName()), "Company", "Taxi")
            end
        end

        self:addLog(driver, "Taxi", (" drove %s (+%s)"):format(customer:getName(), toMoneyString(priceForEPT)))

        if isElement(driver) then
            driver:sendInfo(_("Player %s has exited! The ride earned you %d$", driver, customer:getName(), price))
            if price > 0 then
        ...
        if self.m_TaxiCustomer[customer]["blip"] then
            delete(self.m_TaxiCustomer[customer]["blip"])
        end
...
function PublicTransport:updateTaxometer(customer)
	if self.m_TaxiCustomer[customer] and isElement(customer) and self.m_TaxiCustomer[customer]["driver"] and isElement(self.m_TaxiCustomer[customer]["driver"]) then
		...
		if not self.m_TaxiCustomer[customer]["isFree"] then
			local price = TAXI_PRICE_PER_KM_CAR
			if self.m_TaxiCustomer[customer]["type"] == "Plane" or self.m_TaxiCustomer[customer]["type"] == "Helicopter" then price = TAXI_PRICE_PER_KM_PLANE end
			self.m_TaxiCustomer[customer]["price"] = math.floor(self.m_TaxiCustomer[customer]["diff"] * price)
		end
		...
		if customer:getBankMoney() < self.m_TaxiCustomer[customer]["price"] and not self.m_TaxiCustomer[customer]["moneyWarningSent"] then
			...
			customer:sendWarning(_("You don't have enough money with you!", customer))
			self.m_TaxiCustomer[customer]["driver"]:sendWarning(_("The player no longer has enough money on him!", customer))
		end
...
function PublicTransport:Event_onPlayerQuit()
	if self.m_TaxiCustomer[source] then
		...
		driver:sendError(_("The customer %s has gone offline!", driver, source:getName()))
		self:endTaxiDrive(source)
	end
end

function PublicTransport:Event_OnTaxiDriverModeChange(customer, withTaxometer)
	if not client.m_TaxiData or client.m_TaxiData ~= client.vehicle then return client:sendError(_("You have to be in a taxi!", client)) end
	if not customer.vehicle or customer.vehicle ~= client.m_TaxiData then return client:sendError(_("The player is no longer in the taxi!", client)) end
...
function PublicTransport:Event_setTargetFromMap(posX, posY)
	if self.m_TaxiCustomer[client] and self.m_TaxiCustomer[client]["driver"] then
		...
		driver:sendInfo(_("The customer %s has marked his destination on the map! Destination: %s/%s", driver, client:getName(), getZoneName(posX, posY, 0), getZoneName(posX, posY, 0, true)))
		client:sendInfo(_("You have marked your destination on the map! Destination: %s/%s", client, getZoneName(posX, posY, 0), getZoneName(posX, posY, 0, true)))
...
function PublicTransport:Event_sendTargetTellMessage(posX, posY)
	local driver = self.m_TaxiCustomer[client]["driver"]
	if driver then
		driver:sendInfo(_("The customer %s will inform you of their destination!", driver, source:getName()))
		client:sendInfo(_("Please tell the driver %s your destination", client, driver:getName()))		
	end
end
...
function PublicTransport:Event_changeBusDutyState(state, arg, arg2) -- from clientside mouse menu
	...
	if not PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "company", "startBusTour") then 
		client:sendError(_("You are not authorized to operate a bus line!", client)) 
		return 
	end
...
function PublicTransport:stopBusTour_Driver(player) --also gets triggered when player changes to off-duty
	...
	if player.Bus_Statistics then
		self:addLog(player, "Bus", (" has served line %d (%s stations, +%s$)!"):format(player.Bus_Statistics.line, player.Bus_Statistics.stations, player.Bus_Statistics.money))
...
function PublicTransport:startBusTour_Driver(player, nextStation, line)
	...
	if not nextStation or not self.m_BusStops[nextStation] then
		player:sendShortMessage(_("This bus has no route anymore - If you know what happened to the bus previously (e.g., if the bus driver went offline or off-duty), please report it on the bug tracker", player))
	else
...
function PublicTransport:stopBusTour(vehicle, player)
	...
	for i,v in pairs(vehicle:getOccupants()) do
		if v.vehicleSeat ~= 0 then
			v:removeFromVehicle()
			v:sendWarning(_("This bus is no longer in service on this route.", v))
		end
	end
...
function PublicTransport:BusStop_Hit(player, matchingDimension)
	if getElementType(player) == "player" and matchingDimension and getPedOccupiedVehicleSeat(player) == 0 then
		...
		if vehicle:getSpeed() > 30 then
			player:sendError(_("You are driving too fast!", player))
			return false
		end
		...
		if lastId then
			...
			player:giveCombinedReward("Bus driver salary", {
				money = {
					mode = "give",
					bank = true,
			...
			self.m_BankAccountServer:transferMoney({self, nil, true}, math.round(490 * (dist/1000)), ("Bus journey line %d from %s"):format(line, player:getName()), "Company", "Bus")
		...
		player.Bus_Blip = Blip:new("Marker.png", x, y, player, 9999, PublicTransport.ms_BusLineData[line].color)
		player.Bus_Blip:setZ(z)
		player.Bus_Blip:setDisplayText("Bus Station")
		...
		if isElement(self.m_BusStops[stopId].marker[vehicle]) then
			...
			player:sendError(_("Internal error: Please restart the line", player))
		end
