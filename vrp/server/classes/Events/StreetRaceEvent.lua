...
function StreetRaceEvent:onStart()
	...
	setTimer(
		function()
			...
			self.m_DestinationBlip = Blip:new("Marker.png", pos.x, pos.y,root,9999)
			self.m_DestinationBlip:setColor(BLIP_COLOR_CONSTANTS.Red)
			self.m_DestinationBlip:setDisplayText("Ziel")
			...
			self:sendMessage("The event has been started!", 255, 255, 0)
...
function StreetRaceEvent:onPlayerExit()
	if source:getData("inEvent") then
		source:sendError("You have been disqualified!")
		self:quit(source)
...
function StreetRaceEvent:colShapeHit(hitElement, matchingDimension)
	if getElementType(hitElement) == "player" and matchingDimension and getPedOccupiedVehicleSeat(hitElement) == 0 and self:isMember(hitElement) then
		...
		self:sendMessage("%s has achieved the target as %d.", 255, 255, 0, getPlayerName(hitElement), #self.m_Ranks)
		...
		hitElement:sendSuccess(_("You won the road race %d$!", hitElement, moneyAmount))
		...
		if #self.m_Ranks == 1 then
			self:sendMessage("The first player has reached the goal. You have 1 minute left to reach the goal!", 255, 255, 0)
...
function StreetRaceEvent:getDescription(player)
	return _([[Road Racing against other players
	]], player)
end
