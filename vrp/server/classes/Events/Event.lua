...
function Event:virtual_constructor(Id)
	...
	self.m_EventBlip:setDisplayText("Event")
		...
		function(hitElement, matchingDimension)
			if getElementType(hitElement) == "player" and matchingDimension then
				if not self:isMember(hitElement) then
					...
					hitElement:sendWarning(_("You're already taking part in this event!", hitElement))
				end
			end
		end
	)
		...
		function(hitElement, matchingDimension)
			if getElementType(hitElement) == "player" and matchingDimension then
				if self:isMember(hitElement) then
					...
					hitElement:sendWarning(_("You have left the event zone and are therefore no longer taking part in the event!", hitElement))
...
function Event:join(player)
	self:sendMessage("%s has joined the event!", 255, 255, 0, getPlayerName(player))
...
function Event:start()
	...
	if #self.m_Players <= 2 then
		for k, player in pairs(self.m_Players) do
			player:sendShortMessage(_("The event was canceled due to too few players!", player))
		end
