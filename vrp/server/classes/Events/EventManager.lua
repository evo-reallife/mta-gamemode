...
function EventManager:openEvent(eventClass)
	...
	for k, player in pairs(PlayerManager:getSingleton():getReadyPlayers()) do
		player:sendShortMessage(_("In 5 minutes, a '%s' will start! Head to the flag blip to participate.", player, event:getName()), "San News - Event", {0, 32, 63}, 15000)
	end
...
function EventManager:Event_eventJoin(eventId)
	...
	if not event then
		client:sendError(_("This event no longer exists!", client))
		return
	end

	if #event:getPlayers() >= MAX_PLAYERS_PER_EVENT then
		client:sendError(_("This event is already full!", client))
		return
	end

	event:join(client)
	client:sendShortMessage(_("You have successfully registered for this event. Stay in the neighborhood!", client))
