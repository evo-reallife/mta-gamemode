...
function HalloweenEasterEggs:createGhostShip()
    ...
    PlayerManager:getSingleton():breakingNews(("A strange ship has appeared on the coast of %s!"):format(zone))
end
...
function HalloweenEasterEggs:giveReward(id)
    if client:getId() then
        ...
        if not self.m_DiscoveredAchievents[client:getId()][id] then
            ...
            client:sendSuccess(_("You received %s pumpkins and %s sweets!", client, self.m_Rewards.pumpkins, self.m_Rewards.sweets))
        end
    end
end
