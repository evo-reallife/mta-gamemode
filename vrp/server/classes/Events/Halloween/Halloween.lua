...
Halloween.ms_Phrases = {
	single = {
		...
		"That's for you!",
		"Here you go!",
		"I still have this in the cupboard.",
		"So who's dressed up here?",
		"Enjoy it!",
	},
	multi = {
		"You're a scary bunch!",
		"Let's see if that is enough for you...",
		"Oh, I was so scared! Here you go!",
		"If it's not the neighboring ghosts - here you go!",
		"I need a big bag for that.",
		"Here are the finest sweets!",
	},
}
	...
	if isElement(player) and getElementType(player) == "player" then
		...
		if not d.currentHouseId then
			if not d.visitedHouses[houseId] or (getTickCount() - d.visitedHouses[houseId]) > Halloween.ms_HouseTTCooldown then
				...
				player:triggerEvent("Countdown", time/1000, "Trick or Treat")
				player:sendInfo(_("Screams 'Trick or treat!' to draw the residents' attention to you.", player))
				...
				for i, v in pairs(getElementsByType("player")) do
					...
					if HouseManager:getSingleton().m_Houses[houseId]:isPlayerNearby(v) and not self.m_TrickOrTreatPIDs[v:getId()].currentHouseId and v:getInterior() == 0 and v:getDimension() == 0 then
						if not self.m_TrickOrTreatPIDs[v:getId()].visitedHouses[houseId] or (getTickCount() - self.m_TrickOrTreatPIDs[v:getId()].visitedHouses[houseId]) > Halloween.ms_HouseTTCooldown then
							...
							v:triggerEvent("Countdown", time/1000, "Trick or Treat")
							v:sendInfo(_("Screams 'Trick or treat!' to draw the residents' attention to you.", player))
						else
							v:sendError(_("You've already been here! Come back later.", v))
						end
					end
				end
			else
				player:sendError(_("You've already been here! Come back later.", player))
			end
		end
	end
end
...
function Halloween:checkForTTScream(player, text)
	...
	self:initTTPlayer(pId)
	local d = self.m_TrickOrTreatPIDs[pId]
	if text:lower():gsub("ß", "ss"):find("Trick or Treat") then
					...
					if HouseManager:getSingleton().m_Houses[houseId]:isPlayerNearby(pl) then
						if d.lastMessage and d.lastMessage >= d.trickStarted and (getTickCount() - d.lastVisited) > 20000 then
							if ownerAtHome then
								...
								pl:sendSuccess(_("You have got %d %s!", pl, rnd, rnd > 1 and "Süßigkeiten" or "Süßigkeit"))
								pl:sendMessage(("Resident says: %s"):format(rndPhrase), 200, 200, 200)
							else
								pl:sendShortMessage(_("Nobody seems to be at home...", pl))
							end
							d.visitedHouses[houseId] = getTickCount()
							d.lastVisited = getTickCount()
						end
					else
						pl:sendWarning(_("You have to stay near the door to get sweets!", pl))
					end
				end
...
function Halloween:Event_buyBonus(bonusId)
	...
	if playerSweets < bonus["Sweets"] then
		client:sendError(_("You don't have enough sweets! (%d)", client, bonus["Sweets"]))
		return
	end

	if playerPumpinks < bonus["Pumpkin"] then
		client:sendError(_("You don't have enough pumpkins! (%d)", client, bonus["Pumpkin"]))
		return
	end
	...
	elseif bonus["Type"] == "Item" then
		if client:getInventory():getFreePlacesForItem(bonus["ItemName"]) >= bonus["ItemAmount"] then
			...
			client:sendError(_("You don't have enough space in your inventory!", client))
			return
		end
	elseif bonus["Type"] == "Vehicle" then
		...
		if vehicle then
			...
			client:sendMessage(_("Error creating the vehicle. Please notify an admin!", client), 255, 0, 0)
		end
	...
	elseif bonus["Type"] == "Special" then
		...
		elseif bonus["Text"] == "Payday Bonus" then
			if not client.m_HalloweenPaydayBonus then
				...
				client:sendError(_("You've already activated the Payday Bonus!", client))
				return
			end
		elseif bonus["Text"] == "Nick Change" then
			outputChatBox(_("Please write a ticket to have the nick change carried out by an admin.", client), client, 0, 255, 0)
			outputChatBox(_("Be sure to add that you'll get it for free through the Halloween event!", client), client, 0, 255, 0)
		elseif bonus["Text"] == "Zombie Skin" then
			client:getInventory():giveItem("Kleidung", 1, 310)
			client:sendShortMessage("The zombie skin has been added to your inventory!")
	...
	client:sendSuccess(_("You have successfully purchased the bonus %s for %d pumpkins and %d sweets!", client, bonus["Text"], bonus["Pumpkin"], bonus["Sweets"]))
...
function Halloween:halloweenDmMessage()
	for index, playeritem in pairs(PlayerManager:getSingleton():getReadyPlayers()) do
		playeritem:sendShortMessage(_("At %d:13 a Halloween special starts at the DM Arena!", playeritem, getRealTime().hour), _("Halloween-DM", playeritem), {255, 130, 0})
	end
end
...
function Halloween:halloweenDmStart()
	...
	for index, playeritem in pairs(PlayerManager:getSingleton():getReadyPlayers()) do
		playeritem:sendShortMessage(_("The Halloween Deathmatch Lobby is now open!", playeritem), _("Halloween-DM", playeritem), {255, 130, 0})
	end
end
