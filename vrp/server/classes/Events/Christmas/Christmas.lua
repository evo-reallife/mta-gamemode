...
function Christmas:Event_buyBonus(bonusId)
	...
	if playerPackages < bonus["Packages"] then
		client:sendError(_("You don't have enough packages! (%d)", client, bonus["Packages"]))
		return
	end

	if playerCandyCane < bonus["CandyCane"] then
		client:sendError(_("You don't have enough candy canes! (%d)", client, bonus["CandyCane"]))
		return
	end
	...
	elseif bonus["Type"] == "Points" then
		...
		client:sendShortMessage(_("%d Points Received!", client, bonus["PointsAmount"]))
	elseif bonus["Type"] == "Skin" then
		client:getInventory():giveItem("Kleidung", 1, bonus["SkinId"])
		client:sendShortMessage("The skin has been added to your inventory!")
	elseif bonus["Type"] == "Special" then
		...
		elseif bonus["Text"] == "Nick Change" then
			...
			outputChatBox(_("Be sure to add that you will receive it for free through the Weichnachts event!", client), client, 0, 255, 0)
		elseif bonus["Text"] == "30 Tage VIP" then
			if DEBUG then
				client:sendError(_("Not available in DEBUG mode (test server)!", client))
				return
			end
			client.m_Premium:giveEventMonth()
		elseif bonus["Text"] == "Lebkuchen-Maske" then
			client:getInventory():giveItem("Lebkuchen-Maske", 1)
			client:sendShortMessage("The mask has been added to your inventory!")
		end
	end

	client:getInventory():removeItem("Zuckerstange", bonus["CandyCane"])
	client:getInventory():removeItem("Päckchen", bonus["Packages"])
	client:sendSuccess(_("You have successfully purchased the bonus %s for %d packets and %d candy cane/s!", client, bonus["Text"], bonus["Packages"], bonus["CandyCane"]))
...
function Christmas:openDoor()
	if not self.m_AdventCalender[client:getId()] then
		if client:getInventory():giveItem("Päckchen", 5) then
			client:sendSuccess("You have received 5 parcels!")
			self.m_AdventCalender[client:getId()] = true
		end
	else
		client:sendError("You've already opened the little door!")
	end
end