...
function ChristmasEasterEggs:startPresentSpawnEasterEgg()
    ...
    for i, player in pairs(PlayerManager:getSingleton():getReadyPlayers()) do
        player:triggerEvent("breakingNews", "Is it snowing?", "Christmas news")
        player:triggerEvent("breakingNews", "Santa reportedly lost presents in Los Santos, according to reports.", "Christmas News")
        player:triggerEvent("breakingNews", "So keep your eyes peeled!", "Christmas News")
    end
end