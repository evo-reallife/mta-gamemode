		...
		if getRealTime().monthday == 24 then
			GlobalTimer:getSingleton():registerEvent(function()
				for key, player in pairs(PlayerManager:getSingleton():getReadyPlayers()) do
					...
					player:sendInfo(_("The Cookie Clicker event is now closed. \nInformation about the winners and prizes will be announced in the forum soon.\nThank you to all participants."))
					self.m_IsActive = false
				end
		...
		if playerToBan then
			...
			if Account.getIdFromName(playerToBan) then
				...
				if not self.m_CookieClickerBans[userId] then
					...
					if bPlayer then
						bPlayer:sendError(_("You have been blocked by the Cookie Clicker! Reason: %s", bPlayer, args or ""))
						bPlayer:triggerEvent("CookieClicker:forceCloseGUI", false)
					end
				else
					player:sendError(_("The player already has a ban!", player))
				end
			else
				player:sendError(_("Error: Player not found!", player))
			end
		else
...
function CookieClickerManager:sendCookieData(player)
	if self.m_IsActive then 
		if not self.m_CookieClickerBans[player:getId()] then
			...
			player:sendError(_("You have been excluded from the cookie clicker!\nReason: %s", player, self.m_CookieClickerBans[player:getId()]))
			player:triggerEvent("CookieClicker:forceCloseGUI", true)
		end
	else
		player:sendError(_("Currently no Cookie Clicker event is running!", player))
		player:triggerEvent("CookieClicker:forceCloseGUI", true)
	end
end
