...
function QuestNPCTransport:onVehicleEnter(player, seat)
	if table.find(self:getPlayers(), player) then
		player:sendWarning("Santa's too fat to get into a car!")
	end
end
		...
		if table.find(self:getPlayers(), player) then
			if self.m_Bots[player] then
				if (self.m_Bots[player]:getPosition() - player:getPosition()).length < 30 then
					player:sendSuccess("You have successfully delivered Santa!")
					self:success(player)
				else
					player:sendWarning("Your Santa is too far away, go back into the marker as soon as he is closer to you!")
				end
			end
		end
	end
			...
			if self.m_Bots[player] then
				if (self.m_Bots[player]:getPosition() - player:getPosition()).length > 30 then
					if not player.christmasBotWarning then
						player:sendWarning("Careful, your Santa is too far away from you! Stay with him, otherwise the quest will be canceled!")
						player.christmasBotWarning = true
					else
						player:sendError("You've gone too far away from your Santa!")
...
function QuestNPCTransport:addPlayer(player)
	...
	self.m_Bots[player]:setData("Ped:fakeNameTag", "Santa Claus from "..player:getName(), true)
