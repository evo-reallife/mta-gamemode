...
function DMRaceEvent:onPlayerWasted(player)
	...
	if leftPlayers == 1 then
		...
		winningPlayer:sendSuccess(_("You have won and got %d$!", winningPlayer, money))

		-- Stop event
		delete(self)
	else
		player:sendInfo(_("You have reached the %d. Place and got %d$!", player, leftPlayers+1, money))
	end
end
...
function DMRaceEvent:EndTimer_Expired()
	...
	while #self:getPlayers() ~= 0 do
		...
		killPed(player)
		player:sendShortMessage(_("Time expired!", player))
	end
end
...
function DMRaceEvent:getDescription(player)
    return _([[The game mode is inspired by Race servers with Deathmatch (DM) maps.

        The goal is to reach the destination before everyone else while facing numerous obstacles on the way.
        A single wrecked car leads to defeat.
    ]], player)
end
