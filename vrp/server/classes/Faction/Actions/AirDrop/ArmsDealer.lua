...
function ArmsDealer:checkoutCart(cart)
    ...
    if not PermissionsManager:getSingleton():isPlayerAllowedToStart(client, "faction", "Airdrop") then
		client:sendError(_("You are not authorized to start an airdrop!", client))
		return
	end
	if FactionState:getSingleton():countPlayers() < ARMSDEALER_MIN_MEMBERS then
       return client:sendError(_("There must be at least %d state online!",client, ARMSDEALER_MIN_MEMBERS))
    end
    if client and client.getFaction and client:getFaction() then 
        if cart and not self.m_InAir then
            ...
            if not self.m_Order[faction] then
                ...
                client:getFaction():addLog(client, "Lager", ("Airdrop launched for $%s!"):format(self.m_TotalPrice))
                self.m_LastPlayer = client 
                self.m_LastFaction = faction
            else 
                client:sendError(_("Your faction has already ordered today!", client))
            end
        else 
            client:sendError(_("An airdrop is already on the way!", client))
        end
    end
end
...
function ArmsDealer:processCart( order, faction )
    ...
    PlayerManager:getSingleton():breakingNews("An unidentified airplane has entered the airspace of San Andreas!")
    self.m_InAir = true
end
...
function ArmsDealer:setupPlane(pos, time, faction, order)
    self:sendOperatorMessage(faction, "In the air!")
    ...
    setTimer(function() self:sendOperatorMessage(faction, "Dropping in T-10!") end, (distanceToDrop*acceleration)-10000, 1)
    setTimer(function() 
