		...
		if isTimer(self.m_OpenVaulTimer) then
			...
			if self.m_RobFaction then
				for k, pl in pairs(self.m_CurrentBank:getEvilPeople()) do
					pl:triggerEvent("CountdownStop","Safe opens:")
				end
			end
		end
	end
...
function BankRobberyManager:Event_onStartHacking()
	if client:getFaction() and client:getFaction():isEvilFaction() then
		if self.m_IsBankrobRunning then
			...
			client:sendError(_("There is currently no bank robbery in progress!", client))
		end
	end
end
...
function BankRobberyManager:Event_onHackStepFailed() --somebody hit the wall
	if not self.m_CurrentBank.m_AlarmTriggered then
		for k, player in pairs(self.m_CurrentBank:getEvilPeople()) do
			player:sendWarning(_("%s was careless and alerted the police while hacking!", player, client:getName()))
		end
...
function BankRobberyManager:Event_onHackSuccessful() --all three steps
	...
	for k, player in pairs(self.m_CurrentBank:getEvilPeople()) do
		player:sendSuccess(_("The security system has been cracked by %s! The safe door is open", player, client:getName()))
		player:triggerEvent("Countdown", (openTime/1000), "Safe opens:")
	end
