...
function BankRobbery:destroyRob()
	...
	if tooLatePlayers then
		for key, player in pairs( tooLatePlayers) do
			killPed(player)
			player:sendInfo("You died in the locked room!")
		end
	end
...
function BankRobbery:startRobGeneral(player) --ped got targeted
	...
	if not self.startAlarm then -- this is a function ;)
		...
		faction:sendShortMessage(_("%s has started a robbery!", player, player.name))
		if faction:getAllianceFaction() then
			faction:getAllianceFaction():sendWarning(_("Your alliance partner %s has launched a heist! See how you can support them!", player, faction:getShortName()), "New Action", true, self.m_MarkedPosition)
		end
		...
		faction:sendShortMessage(_("%s has started a robbery! You have 5 minutes before the SAPD is alerted!", player, player.name))
		if faction:getAllianceFaction() then
			faction:getAllianceFaction():sendWarning(_("Your alliance partner %s has launched a heist! See how you can support them!", player, faction:getShortName()), "New Action", true, self.m_MarkedPosition)
		end
	end
	...
	if faction and faction:isEvilFaction() then
		if attacker:isFactionDuty() then
			...
			if FactionState:getSingleton():countPlayers() < self.ms_MinBankrobStateMembers and not DEBUG then
				attacker:sendError(_("There must be at least %d state online!",attacker, self.ms_MinBankrobStateMembers))
				return false
			end
			if not PermissionsManager:getSingleton():isPlayerAllowedToStart(attacker, "faction", "BankRobbery") then
				attacker:sendError(_("You are not authorized to start a bank robbery!",attacker))
				return false
			end
			if self:getDifficulty() < 1 then
				attacker:sendError(_("There is still not enough money in the vaults!",attacker))
				return false
			end
			self:startRob(attacker)
			outputChatBox(_("Bank employee says: Help! I'll open the door to the vault for you!", attacker), attacker, 255, 255, 255)
			outputChatBox(_("Bank employee says: Please don't hurt me!", attacker), attacker, 255, 255, 255)
		else
			attacker:sendError(_("Only members of the evil factions can rob the bank!", attacker))
		end
	else
		attacker:sendError(_("Only members of an evil faction can rob the bank!", attacker))
	end
end

function BankRobbery:timeUp()
	PlayerManager:getSingleton():breakingNews("The bank robbery is over! The perpetrators took too much time!")
	Discord:getSingleton():outputBreakingNews("The bank robbery is over! The perpetrators took too much time!")
	self:destroyRob()
end
...
function BankRobbery:updateBreakingNews()
	...
	if rnd == 1 then
		msg =  ("The %s is still in progress!"):format(self.m_Name == "Casino" and "Casinoüberfall" or "Bank rob")
	elseif rnd == 2 then
		...
		if self.m_BrNe_EvilPeople > nowEvilPeople then
			msg = ("According to the latest information, there are only %d robbers left on the site!"):format(nowEvilPeople)
			self.m_BrNe_EvilPeople = nowEvilPeople
		elseif self.m_BrNe_EvilPeople < nowEvilPeople then
			msg = ("The SAPD now assumes that %d robbers are involved!"):format(nowEvilPeople)
			self.m_BrNe_EvilPeople = nowEvilPeople
		elseif self.m_BrNe_EvilPeople == nowEvilPeople then
			msg = ("The situation %s unchanged. %d Robbers are on the premises!"):format(type, nowEvilPeople)
			self.m_BrNe_EvilPeople = nowEvilPeople
		end
	elseif rnd == 3 then
		...
		if self.m_BrNe_StatePeople > nowStatePeople then
			msg = ("The SAPD is only on site with %d officers!"):format(nowStatePeople)
			self.m_BrNe_StatePeople = nowStatePeople
		elseif self.m_BrNe_StatePeople < nowStatePeople then
			msg = ("The SAPD has called in additional units. There are %d officers on site!"):format(nowStatePeople)
			self.m_BrNe_StatePeople = nowStatePeople
		elseif self.m_BrNe_StatePeople == nowStatePeople then
			msg = ("The situation %s unchanged. %d officers are on the premises!"):format(type, nowStatePeople)
			self.m_BrNe_StatePeople = nowStatePeople
		end
	elseif rnd == 4 then
		msg = ("According to the latest information, the perpetrators are members of the %s!"):format(self.m_RobFaction:getName())
	end
	...
	if button == "left" and state == "down" then
		if self:isPlayerParticipant(player) and player:getFaction():isEvilFaction() then
			if self.m_IsBankrobRunning then
				...
				player:sendError(_("There is currently no bank robbery in progress!", player))
			end
		end
	end
end
	...
	if button == "left" and state == "down" then
		if getDistanceBetweenPoints3D(player:getPosition(), source:getPosition()) < 3 then
			if self:isPlayerParticipant(player) then
				...
				player:sendError(_("You are not involved in this robbery!", player))
			end
		else
...
function BankRobbery:addMoneyToBag(player, money)
	for i, bag in pairs(self.m_MoneyBags) do
		if bag:getData("Money") + money < self.ms_MoneyPerBag then
			...
			player:sendShortMessage(_("%d$ packed in the money bag %d!", player, money, i))
			return true
		end
	end
	if not self.ms_BagSpawns[#self.m_MoneyBags+1] then
		player:sendError(_("You already have the maximum number of money bags!", player))
		return false
	end
	...
	player:sendShortMessage(_("%d$ packed in the money bag %d!", player, money, #self.m_MoneyBags))
...
function BankRobbery:Event_OnTruckStartEnter(player, seat)
	if seat == 0 and not self:isPlayerParticipant(player) then
		player:sendError(_("Only factions can drive the bank robbery truck!", player))
		cancelEvent()
	end
end
...
function BankRobbery:handleBagDelivery(faction, player)
	...
	if player.vehicle then return player:sendInfo(_("You have to hand over the money bags!", player)) end

	local money = bag:getData("Money")
	if bag:getModel() == 1550 and money and money > 0 then
		self.m_BankAccountServer:transferMoney({"faction", faction:getId(), true}, money, "Bankrob money bag", "Action", "BankRobbery", {silent = true})
		if faction:isStateFaction() then
			FactionState:getSingleton():sendShortMessage(_("%s has secured %s loot!", player, player:getName(), toMoneyString(money)))
		else
			local text = _("%s has brought %s to safety!", player, player:getName(), toMoneyString(money))
	...
	if self.m_SafeDoor.m_Open then
		if self:getRemainingBagAmount() == 0 then
			local text = ("The robbery was successfully completed! %s"):format(faction:isStateFaction() and "The money was recovered!" or "The perpetrators escaped with the loot!")
