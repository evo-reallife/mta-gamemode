...
function BankPalomino:startRob(player)
	...
	PlayerManager:getSingleton():breakingNews("A currently unknown faction raids the Palomino Creek Bank!")
	Discord:getSingleton():outputBreakingNews("A currently unknown faction raids the Palomino Creek Bank!")
	FactionState:getSingleton():sendWarning("The bank in Palomino Creek is being robbed!", "New Alert", true, {2318.43, 11.37, 26.48})
...
function BankPalomino:BombArea_Place(bombArea, player)
	if not player:getFaction() then
		player:sendError(_("You can only rob banks if you are a member of an evil faction", player))
		return false
	end
	...
	if not PermissionsManager:getSingleton():isPlayerAllowedToStart(player, "faction", "BankRobbery") then
		player:sendError(_("You are not authorized to start a bank robbery!", player))
		return false
	end
	
	if not DEBUG and FactionState:getSingleton():countPlayers() < 3 then
		player:sendError(_("At least 3 state must be online to start the raid!", player))
		return false
	end
