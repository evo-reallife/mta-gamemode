...
JewelryStoreRobbery.StartMessages = {
    "A jewelry store reports a robbery!",
    "The jeweler in the west of Los Santos is being robbed!",
    "Unknown individuals triggered the alarm of a jeweler!",
    "An emergency call from a jeweler reached the Police Department!"
}

JewelryStoreRobbery.EscapeMessages = {
    "The robbers are still on the run!",
    "The police continues to pursue the robbers of the jeweler!",
    "The robbers seem to be attempting to flee the city!",
    "The robbers are still on the run!",
    "The pursuit of the jewelry store robbers continues!"
}
...
function JewelryStoreRobbery:constructor(attacker, maxBags)
	...
	FactionState:getSingleton():sendWarning("A jeweller is robbed!", "New Application", true, Vector3(561.292, -1506.786, 14.548))
	...
	self.m_EvilDeliveryPed:setData("Ped:greetText", "You look like someone who wants to get rid of something!")
	...
	self.m_EvilDeliveryPedBlip:setDisplayText("Jeweller-Robbery Tax")
	...
	self.m_StateDeliveryPedBlip:setDisplayText("Jeweller-Robbery Tax State")
	...
	for index, player in pairs(JewelryStoreRobberyManager:getSingleton().m_Players) do
		if isElement(player) then
			...
			player:sendShortMessage(_("Press '%s' to smash a glass cabinet!", player, "F"))
		end
	end
...
function JewelryStoreRobbery:Event_BreakGlass(player)
	...
	if player.m_PlayerAttachedObject and not player.m_PlayerAttachedObject.m_Jewelry then
		player:sendError(_("You're already wearing an object!", player))
		return
	end
	...
	if nearestShelveDistance < 0.7 then
		...
		local timer = setTimer(function()
			...
			if not shelve.m_Looted then
				if player.m_PlayerAttachedObject and not player.m_PlayerAttachedObject.m_Jewelry then
					player:sendError(_("You already have an object with you!", self))
					return
				end
		...
		if getDistanceBetweenPoints3D(player:getPosition(), source:getPosition()) < 3 then
			if (player:getFaction():isStateFaction() or player:getFaction():isEvilFaction()) and player:isFactionDuty() then
				if getElementData(player, "heligrab.vehicle") or player.vehicle then
					player:sendError(_("You can't pick up the loot while you are in a vehicle!", player))
					return
				end
				...
				if player.m_PlayerAttachedObject and player.m_PlayerAttachedObject.m_Jewelry then
					...
					player:sendShortMessage(_("The loot has been added to your bag!", player))
					source:destroy()
					return
				end
				player:attachPlayerObject(source)
			else
				player:sendError(_("You're not involved in this robbery!", player))
			end
		else
			player:sendError(_("You're too far away from the prey!", player))
		end
	end
end
		...
		if getDistanceBetweenPoints3D(player:getPosition(), source:getPosition()) < 3 then
			if player:getFaction():isEvilFaction() and player:isFactionDuty() then
				if player.m_PlayerAttachedObject and player.m_PlayerAttachedObject.m_Jewelry then
					...
					player:sendSuccess(_("You've handed over the loot!!", player))
					bag:destroy()
					...
					player:sendError(_("You don't have any loot with you!", player))
				end
			else
				player:sendError(_("You can't give away any loot!", player))
			end
		else
			player:sendError(_("You're too far away!", player))
		end
	end
end
		...
		if getDistanceBetweenPoints3D(player:getPosition(), source:getPosition()) < 3 then
			if player:getFaction():isStateFaction() and player:isFactionDuty() then
				if player.m_PlayerAttachedObject and player.m_PlayerAttachedObject.m_Jewelry then
					...
					player:sendError(_("You don't have any loot!", player))
				end
			else
				player:sendError(_("You can't hand in any loot!", player))
			end
		else
			player:sendError(_("You're too far away!", player))
		end
...
function JewelryStoreRobbery:updateBreakingNews()
	...
	if evilCount > 0 then
		if evilCount > stateCount then
			...
			if messageId == 1 then
				PlayerManager:getSingleton():breakingNews("Some robbers seem to belong to the %s faction!", self.m_Faction:getName())
			elseif messageId == 2 then
				PlayerManager:getSingleton():breakingNews("According to surveillance cameras, %d perpetrators are in the jeweller's!", evilCount)
			else
				PlayerManager:getSingleton():breakingNews("Avoid the shops in the West of the City!")
			end
		else
			PlayerManager:getSingleton():breakingNews("There are Currently %d officers in the jeweller's!", stateCount)
		end
	else
		if not self.m_Escaped then
			PlayerManager:getSingleton():breakingNews("The Perpetrators have stolen loot and are on the run!")
			self.m_Escaped = true
		else
