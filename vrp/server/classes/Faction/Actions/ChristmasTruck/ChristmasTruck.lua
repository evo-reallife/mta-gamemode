...
function ChristmasTruck:constructor(driver)
	...
	for i, faction in pairs(FactionManager:getSingleton():getAllFactions()) do
		if not faction:isRescueFaction() then
			...
			self.m_DestinationBlips[faction:getId()]:setDisplayText(("Christmas-Tree (%s)"):format(faction:getShortName()))
...
function ChristmasTruck:Event_OnChristmasTruckDestroy()
	if self and not self.m_Destroyed then
		...
		PlayerManager:getSingleton():breakingNews("The Christmas truck was destroyed!")
		Discord:getSingleton():outputBreakingNews("The Christmas truck was destroyed!")
		delete(self)
	end
end
...
function ChristmasTruck:timeUp()
	delete(self)
	PlayerManager:getSingleton():breakingNews("The presents could not be put under the tree in time! (time expired)")
	Discord:getSingleton():outputBreakingNews("The presents could not be put under the tree in time! (time expired)")
end

function ChristmasTruck:Event_OnChristmasTruckStartEnter(player,seat)
	if seat == 0 and not player:getFaction() then
		player:sendError(_("Only factions can drive the Christmas truck!",player))
		cancelEvent()
	end
end
	...
	if button == "left" and state == "down" then
		...
		if player:getFaction() and (player:getFaction():isStateFaction() or player:getFaction():isEvilFaction()) then
			if getDistanceBetweenPoints3D(player:getPosition(), source:getPosition()) < 3 then
				...
				player:sendError(_("You are too far away from the gift!", player))
			end
		else
			player:sendError(_("Only factions can pick up gifts!",player))
		end
	end
end
	...
	if hitElement:getType() == "player" and matchingDimension then
		...
		if faction then
			...
			if box then
				...
				hitElement:sendError(_("You didn't bring a present!",hitElement))
			end
		end
	end
end
...
function ChristmasTruck:loadBoxOnChristmasTruck(player,box)
	...
	if presentsOnTruck >= 8 then
		player:sendInfo(_("All presents loaded! The truck is ready!",player))
		self.m_Truck:setFrozen(false)
		self.m_Truck:setLocked(false)
		if isElement(self.m_LoadMarker) then self.m_LoadMarker:destroy() end
	else
		player:sendInfo(_("%d/%d Gifts loaded!", player, presentsOnTruck, 8))
	end
end
	...
	if not self.m_IsSubmerged then
		if isElementInWater(self.m_Truck) then
			...
			self.m_WaterNotificationTimer = setTimer(
				function()
					PlayerManager:getSingleton():breakingNews("The latest sources report that the gifts ended up in the water due to an accident with the transport vehicle!")
				end
			, 180000, 1)
...
function ChristmasTruck:Event_LoadPresent(veh)
	if client:getFaction() then
		if veh == self.m_Truck or VEHICLE_BOX_LOAD[veh.model] then
				...
				if not client.vehicle then
					...
					if self:getAttachedBoxes(veh) < VEHICLE_BOX_LOAD[veh.model]["count"] then
						if box then
							...
							client:sendError(_("You don't have a box with you!",client))
						end
					else
						client:sendError(_("The vehicle is already fully loaded!",client))
					end
				else
			...
			client:sendError(_("This vehicle cannot be loaded!",client))
		end
	else
		client:sendError(_("Only factions can unload crates!",client))
	end
end
			...
			if getDistanceBetweenPoints3D(veh.position, client.position) < 7 then
				if not client:getPlayerAttachedObject() then
					if not client.vehicle and not client:isDead() then
						...
						client:sendError(_("There are no presents on the truck!",client))
						return
					else
						client:sendError(_("You are not allowed to sit in any vehicle!",client))
					end
				else
					client:sendError(_("You already have a gift with you!",client))
				end
			else
				client:sendError(_("You're too far away from the truck!",client))
			end
		else
			client:sendError(_("No gifts can be unloaded from this vehicle!",client))
		end
	else
		client:sendError(_("Only factions can unload gifts!",client))
	end
end
...
function ChristmasTruck:onPresentDeliver(player, tree)
	...
	if isPedInVehicle(player) then
		player:sendInfo(_("Please get out to drop off the presents!", player))
		return
	end
	if not faction then
		player:sendInfo(_("Only factionists can put presents under the tree!", player))
		return
	end
	if player:getPlayerAttachedObject() and player:getPlayerAttachedObject():getData("ChristmasTruck:Present") then
		if self:getAttachedPresents(player) > 0 then
			...
			PlayerManager:getSingleton():breakingNews("Gift %d from %d was placed under the Christmas tree with %s!", self.m_PresentCount-self:getRemainingPresentAmount()+1, self.m_PresentCount, faction:getShortName())
			player:sendInfo(_("You've put a present under the Christmas tree!",player))
			player:detachPlayerObject(box)
			box.Present:destroy()
			box:destroy()
			...
			if table.size(ChristmasTruckManager:getSingleton().m_FactionPresents[player:getFaction():getId()]) < ChristmasTruckManager.MaxPresents then
				...
				player:sendError(_("There's no more room for presents under your Christmas tree", player))
			end
		end
	end
