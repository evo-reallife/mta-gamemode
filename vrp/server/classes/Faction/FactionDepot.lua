...
function Depot.initalize()
	...
	addEventHandler("itemDepotAdd", root, function(id, item, amount)
		if Depot.Map[id] then
			Depot.Map[id]:addItem(client, item, math.floor(amount))
		end
	end)
	...
	addEventHandler("equipmentDepotAdd", root, function(id, item, amount)
		if Depot.Map[id] then
			Depot.Map[id]:addEquipment(client, item, math.floor(amount))
		end
	end)

	addEventHandler("equipmentDepotTake", root, function(id, item, amount)
		if Depot.Map[id] then
			Depot.Map[id]:takeEquipment(client, item, math.floor(amount))
		end
	end)
end
--//
...
function Depot:takeWeaponsFromDepot(player,weaponTable)
			...
			if amount > 0 then
				...
				if typ == "Waffe" then
					if self.m_Weapons[weaponID]["Waffe"] >= amount then
						...
						player:sendError(_("There are not enough %s in stock (%s)!", player, WEAPON_NAMES[weaponID], amount))
					end
				elseif typ == "Munition" then
					...
					if playerWeapons[weaponID] then
						if self.m_Weapons[weaponID]["Munition"] >= amount then
							...
							player:sendError(_("There are not enough %s magazines in the storage (%s)!", player, WEAPON_NAMES[weaponID], amount))
						end
					else
						player:sendError(_("You don't have an %s for a magazine!", player, WEAPON_NAMES[weaponID]))
					end
				end
	...
	local textForPlayer = "You have taken the following weapons from storage:"
	for i,v in pairs(logData) do
		...
		if v > 1 then
			textForPlayer = textForPlayer.. " with ".. (v-1) .. " Magazine(s)"
		end
	end
		...
		if self.m_Items[i]["Item"] == 0 then
			if item ~= "Kleidung" then --wtf we cant check if the item has a value wtf wtf wtf - MasterM 2017
				if giveItemFromServer or player:getInventory():removeItem(item, amount) then
					...
					player:sendInfo(_("You have put %d %s in the depot (slot %d)!", player, amount, item, i))
					player:triggerEvent("ItemDepotRefresh", self.m_Id, self.m_Items)
					StatisticsLogger:getSingleton():addItemDepotLog(player, self.m_Id, item, amount)
					self.m_Owner:addLog(player, "Itemlager", ("has put %s %s in storage!"):format(amount, item))
					return
				else
					player:sendError(_("You don't have enough %s!", player, item))
					return
				end
			else
				player:sendError(_("You can't store this item!", player, item))
				return
			end
		end
	end
	player:sendError(_("There is no free item slot in this depot!", player))
end
		...
		if self.m_Items[slotId]["Item"] ~= 0 then
			--if self.m_Items[slotId]["Amount"] > 0 then
				...
				if player:getInventory():getFreePlacesForItem(item) >= amount then
					...
					player:sendInfo(_("You took %d %s from the depot (slot %d)!", player, amount, item, slotId))
					player:triggerEvent("ItemDepotRefresh", self.m_Id, self.m_Items)
					StatisticsLogger:getSingleton():addItemDepotLog(player, self.m_Id, item, -amount)
					self.m_Owner:addLog(player, "Itemlager", ("has taken %s %s from storage!"):format(amount, item))
					return
				else
					player:sendError(_("You don't have enough space in your inventory!", player))
				end
			--else
			--	player:sendError("Internal Error Amount to low", player)
			--end
		else
			player:sendError(_("You don't have an item in this slot!", player))
		end
	end
end
...
function Depot:addEquipment(player, item, amount, forceSpawn)
	...
	if self.m_Equipments then
		...
		or (amount==-1 and self:removeAllEquipment(player, item, allAmount)) then
			...
			StatisticsLogger:getSingleton():addItemDepotLog(player, self.m_Id, item, -amount)
			self.m_Owner:addLog(player, "Itemlager", ("has put %s %s in the storage!"):format(amount, item))
			player:triggerEvent("ItemEquipmentRefresh", self.m_Id, self.m_Equipments, ArmsDealer.Data)
			player:sendInfo(_("You have placed %d %s in the depot!", player, amount, item))
			return
		else
			player:sendError(_("You don't have enough %s!", player, item))
		end
...
function Depot:takeEquipment(player, item, amount)
	...
	if not player:isFactionDuty() then return player:sendError(_("You're On Duty", player)) end
			...
			if perms then
				if perms[item] then
					if perms[item] > player:getFaction():getPlayerRank(player) then
						return player:sendError(_("No authorization until rank %i or higher!", player, perms[item]))
					end
				end
			end
		end
...
function Depot:checkDistanceFromEquipment(player)
	...
	and (player.m_LastEquipmentDepot:getDimension() == player:getDimension()) then
		return (player:getPosition() - player.m_LastEquipmentDepot:getPosition()):getLength() < 8 or player:sendError("Too far away!")
	end
	return false
end
