...
function FactionState:constructor()
	...
	addCommandHandler("rtr",bind(self.Command_stvo, self))
...
function FactionState:createSelfArrestMarker( pos, int, dim )
			...
			if getElementType(hE) == "player" then
				if hE:getWanteds() > 0 and not (hE:getFaction() and hE:getFaction():isStateFaction() and hE:isFactionDuty()) then
					if hE:getLastGotWantedLevelTime() + 180000 < getTickCount() then
						...
						hE:sendError(_("You must not turn yourself in yet!", hE))
					end
				end
			end
		end
	...
	ElementInfo:new(marker, "Job-Marker")
end
...
function FactionState:createEquipmentEvidence( pos, int, dim, rot )
	...
	ElementInfo:new(box, "Equipment-Box", 2)
end
...
function FactionState:Event_OnEvidenceEquipmentClick(button, state, player)
	if button == "left" and state == "down" then
		if player:getFaction() and player:getFaction():isStateFaction() then
			...
			player:sendError(_("This depot doesn't belong to your faction!", player))
		end
	end
end
...
function FactionState:putEvidenceInDepot(player, box)
	...
	if type == "Waffe" then
		if id then
			player:getFaction():sendShortMessage(("%s has confiscated %s weapon/s [ %s ] (%s $)!"):format(player:getName(), amount, product, price))
			StateEvidence:getSingleton():addWeaponsToEvidence(player, id, amount, true)
		end
	elseif type == "Munition" then
		if id then
			player:getFaction():sendShortMessage(("%s has confused %s ammunition [ %s ] (%s $)!"):format(player:getName(), amount, product, price))
			StateEvidence:getSingleton():addMunitionToEvidence(player, id, amount, true)
		end
	else
		player:getFaction():sendShortMessage(("%s has confesed %s piece %s (%s $)!"):format(player:getName(), amount, product, price))
		self.m_BankAccountServer:transferMoney(player:getFaction(), price , "Black-Market Goods", "Faction", "Black-Market Goods")
	end
...
function FactionState:Event_OnConfirmSelfArrest()
	...
	self:sendMessage("Player "..client:getName().." has surrendered!", 0, 0, 200)
end
...
function FactionState:loadLSPD(factionId)
	...
	elevator:addStation("Roof - Heliports", Vector3(1564.84, -1666.84, 28.40), 90, 0, 0)
	elevator:addStation("Interior", Vector3(237.89, 114.82, 1010.22), 84, 10, 0)
	elevator:addStation("UG Garage", Vector3(1525.16, -1678.17, 5.89), 270, 0, 5) -- Under Ground
...
function FactionState:loadFBI(factionId)
	...
	elevator:addStation("Ground Floor", Vector3(296.49, -36.23, 1032.20), 90, 10)
			...
			if hitElement.vehicle then
				if hitElement:isFactionDuty() and hitElement:getFaction() and hitElement:getFaction():isStateFaction() == true then
					...
					if instanceof(veh, FactionVehicle) and veh:getFaction():isStateFaction() then
						...
						hitElement:sendError(_("Invalid vehicle!", hitElement))
					end
				else
					hitElement:sendError(_("You're not on duty!", hitElement))
				end
			else
				hitElement:sendError(_("You need a vehicle to load it!", hitElement))
			end
		end
	end)
	ElementInfo:new(pickup, "Equipment")
end
						...
						if source ~= targetPlayer then
							if targetPlayer:getWanteds() <= 3 then
								if targetPlayer:getMoney() >= TICKET_PRICE*targetPlayer:getWanteds() + 500 then
									...
									source:sendSuccess(_("You have offered %s a ticket for %d$!", source,  targetPlayer:getName(), TICKET_PRICE*targetPlayer:getWanteds()+500 ))
								else
									source:sendError(_("%s doesn't have enough money! (%d$)", source, targetPlayer:getName(),  TICKET_PRICE*targetPlayer:getWanteds()+500 ))
								end
							else
								source:sendError("The player has no or too high a wanted level!")
							end
						else
							source:sendError("You can't offer yourself a ticket!")
						end
					else
						source:sendError("You're too far away!")
					end
				else
					source:sendError("You're not on duty!")
				end
			else
				source:sendError("Target not found!")
			end
		end
	else outputChatBox(_("Syntax: For civil servants -> /ticket [goal]", source), source, 200, 0,0)
	end
end
		...
		if client:getMoney() >=  TICKET_PRICE*client:getWanteds()+500  then
			if client:getWanteds() <= 3 then
				if cop and isElement(cop) then
					cop:sendSuccess(_("%s has accepted your ticket and paid!", cop, client:getName()))
					self.m_BankAccountServer:transferMoney(cop:getFaction(),  TICKET_PRICE*client:getWanteds()+500 , "Ticket", "Faction", "Ticket")
				end
				client:sendSuccess(_("You have accepted the ticket! You have been granted %s Wanted(s)!", client, client:getWanteds()))
				client:transferMoney(self.m_BankAccountServer,  TICKET_PRICE*client:getWanteds()+500 , "[SAPD] Deposit Ticket", "Faction", "Ticket")
				client:setWanteds(0)
			end
		end
	end
		...
		if type(target) == "string" then
			...
			if targetPlayer then
				if not targetPlayer:getPublicSync("supportMode") then
									...
									if source:isFactionDuty() then
										if not targetPlayer.vehicle then
											if not targetPlayer:isStateCuffed() then
												...
												source:sendError("The player is already tied up!")
											end
										else
											source:sendError("You can't get to the player!")
										end
									else
										source:sendError("You're not on duty!")
									end
								else
									source:sendError("You can't tie yourself up!")
								end
							else
								source:sendError("You don't have any handcuffs!")
							end
						end
					else
						source:sendError("You're too far away!")
					...
					source:sendError("You can't tie up an admin in support mode!")
				end
			else
				source:sendError("Target not found!")
			end
		end
	else outputChatBox(_("Syntax: /cuff [target]", source), source, 200, 0,0)
	end
end
...
function FactionState:Command_uncuff( source, cmd, target )
			...
			if targetPlayer then
				if getDistanceBetweenPoints3D(source:getPosition(), targetPlayer:getPosition()) <= 5 then
					...
					if faction then
							...
							if faction:isStateFaction() then
								if source:isFactionDuty() then
									if targetPlayer:isStateCuffed() then
										...
										source:sendError("The player is not tied up!")
									end
								else
									source:sendError("You don't have any handcuffs!")
								end
						...
						source:sendError("You can't unleash yourself!")
					end
				else
					source:sendError("You're too far away!")
				end
	...
	else outputChatBox(_("Syntax: /uncuff [target]", source), source, 200, 0,0)
	end
end
...
function FactionState:createArrestZone(x, y, z, int, dim)
	...
	ElementInfo:new(pickup, "Locking-Up", 1)
end
...
function FactionState:getFullReasonFromShortcut(reason)
	local amount = false
	if string.lower(reason) == "dp" then
        reason = "Drug Possession (>= 10)"
        amount = 1
    elseif string.lower(reason) == "dp2" then
        reason = "Drug Possession (>= 50)"
        amount = 2
    elseif string.lower(reason) == "dp3" then
        reason = "Drug Possession (>= 150)"
        amount = 3
    elseif string.lower(reason) == "isl" or string.lower(reason) == "insulting" then
        reason = "Insulting"
        amount = 1
	elseif string.lower(reason) == "block" then
        reason = "Obstructing Emergency Services"
        amount = 1
	elseif string.lower(reason) == "stress" then
        reason = "Harassment"
        amount = 1
    elseif string.lower(reason) == "rof" then
        reason = "Refusal Of Orders"
        amount = 1
	elseif string.lower(reason) == "pew" or string.lower(reason) == "firework" then
        reason = "Use of fireworks"
        amount = 1
    elseif string.lower(reason) == "tf" or string.lower(reason) == "theft" then
        reason = "Theft"
        amount = 1
    elseif string.lower(reason) == "epn" or string.lower(reason) == "noise" then
        reason = "Excitement Of Public Nuisance"
        amount = 1
    elseif string.lower(reason) == "dwl" then
        reason = "Driving without a license"
        amount = 1
	elseif string.lower(reason) == "alc" then
        reason = "Driving under the influence of alcohol"
        amount = 1
    elseif string.lower(reason) == "har" or string.lower(reason) == "driver" then
        reason = "Hit & run"
        amount = 1
    elseif string.lower(reason) == "illegal" or string.lower(reason) == "illad" then
        reason = "Illegal advertising"
        amount = 1
	elseif string.lower(reason) == "illrace" or string.lower(reason) == "race" then
        reason = "Illegal Street Racing"
        amount = 1
    elseif string.lower(reason) == "dtp" then
        reason = "Damage To Property"
        amount = 1
    elseif string.lower(reason) == "at" then
        reason = "Attempted Theft"
        amount = 1
    elseif string.lower(reason) == "channel" or string.lower(reason) == "sewage" then
        reason = "Entering The Sewage System"
        amount = 2
    elseif string.lower(reason) == "bribe" or string.lower(reason) == "ab" then
        reason = "Attempted Bribery"
        amount = 2
    elseif string.lower(reason) == "atsv" then
        reason = "(Attempted) Theft Of State Vehicles"
        amount = 2
	elseif string.lower(reason) == "duc" then
        reason = "Drug Cultivation"
        amount = 2
	elseif string.lower(reason) == "du" then
        reason = "Drug Use"
        amount = 2
    elseif string.lower(reason) == "tw" then
        reason = "Threatening with a weapon"
        amount = 2
    elseif string.lower(reason) == "tr" or string.lower(reason) == "threat" then
        reason = "Threat"
        amount = 2
    elseif string.lower(reason) == "press" or string.lower(reason) == "bm" then
        reason = "Blackmail"
        amount = 2
    elseif string.lower(reason) == "flees" or string.lower(reason) == "esc" then
        reason = "Escape from / Out Of Control"
        amount = 2
	elseif string.lower(reason) == "trff" or string.lower(reason) == "trr" or string.lower(reason) == "tipi" then
        reason = "Trafficking in Prohibited items/drugs"
        amount = 2
    elseif string.lower(reason) == "aab" then
        reason = "Assault and Battery"
        amount = 2
    elseif string.lower(reason) == "rtai" or string.lower(reason) == "ri" then
        reason = "He Refused to Allow Inspection"
        amount = 2
    elseif string.lower(reason) == "deceive" or string.lower(reason) == "pretend" or string.lower(reason) == "fff" then
        reason = "Faking False Facts"
        amount = 2
	elseif string.lower(reason) == "sow" then
        reason = "Sale Of Weapons"
        amount = 2
    elseif string.lower(reason) == "cdr" or string.lower(reason) == "auto" or string.lower(reason) == "car" then
        reason = "Car Dealership Robbery"
        amount = 3
	elseif string.lower(reason) == "bo" or string.lower(reason) == "wu" then
        reason = "Bombardment/Use Of Weapons"
        amount = 3
    elseif string.lower(reason) == "weapons" or string.lower(reason) == "pw" then
        reason = "Possession of Illegal Weapons"
        amount = 3
	elseif string.lower(reason) == "eaa" then
        reason = "Entering the army area"
        amount = 3
    elseif string.lower(reason) == "pd" or string.lower(reason) == "garage" then
        reason = "Entering the police station/garage"
        amount = 3
	elseif string.lower(reason) == "sapd" then
        reason = "Entering the SAPD backyard"
        amount = 3
    elseif string.lower(reason) == "house" or string.lower(reason) == "hr" then
        reason = "House Robbery"
        amount = 3
    elseif string.lower(reason) == "bomb" then
        reason = "Carrying Explosives"
        amount = 3
    elseif string.lower(reason) == "rob" then
        reason = "Robbery"
        amount = 3
    elseif string.lower(reason) == "sr" or string.lower(reason) == "shop" then
        reason = "Store Robbery"
        amount = 3
    elseif string.lower(reason) == "atm" then
        reason = "Sabotage of ATMs"
        amount = 3
	elseif string.lower(reason) == "exe" then
        reason = "Executive action"
        amount = 4
	elseif string.lower(reason) == "mord" or string.lower(reason) == "murder" then
        reason = "Murder"
        amount = 4
	elseif string.lower(reason) == "sniper" then
        reason = "Sniper Possession"
        amount = 4
	...
	elseif string.lower(reason) == "fbi" then
        ...
        amount = 5
	elseif string.lower(reason) == "ct" then
        reason = "Robbery on cash transport"
        amount = 5
    elseif string.lower(reason) == "wt" then
        reason = "Weapons Truck"
        amount = 5
    ...
    elseif string.lower(reason) == "swt" then
        reason = "Attack on the State Weapons Truck"
        amount = 6
    ...
    elseif string.lower(reason) == "ht" then
        reason = "Hostage-Taking"
        amount = 6
    elseif string.lower(reason) == "br" or string.lower(reason) == "cr" then
        reason = "Bank/Casino Robbery"
        amount = 6
    elseif string.lower(reason) == "juwe" or string.lower(reason) == "jr" then
        reason = "Robbery at a Jewelry Store"
        amount = 6
	elseif string.lower(reason) == "jail" or string.lower(reason) == "hack" then
		reason = "Prison Escape"
		amount = 12
    elseif string.lower(reason) == "esapd" then
        reason = "Escaping the Execution of a Police Job"
		amount = 12
    elseif string.lower(reason) == "aaco" then
        reason = "Aiding and Abetting a Criminal Offense"
        amount = false
	end
...
function FactionState:sendShortMessage(text, ...)
	for k, player in pairs(self:getOnlinePlayers()) do
		if player:isFactionDuty() then
			player:sendShortMessage(_(text, player), "State", {11, 102, 8}, ...)
		end
	end
end
...
function FactionState:sendMessageWithRank(text, r, g, b, minRank, withPrefix, ...)
	...
	local prefix = withPrefix and "[State] #ffffff" or ""
...
function FactionState:sendShortMessageWithRank(text, minRank, title, color, ...)
	...
	for k, player in pairs(self:getOnlinePlayers()) do
		if player:getFaction():getPlayerRank(player) >= (minRank or 0) then
			player:sendShortMessage(text, "State" .. suffix, color, ...)
		end
	end
end
...
function FactionState:sendStateChatMessage(sourcePlayer, message)
	if not getElementData(sourcePlayer, "StateChatEnabled") then return sourcePlayer:sendError(_("You have deactivated the state chat!", sourcePlayer)) end
	...
	if faction and faction:isStateFaction() == true then
		...
		for k, player in pairs(self:getOnlinePlayers()) do
			if getElementData(player, "StateChatEnabled") then
				player:sendMessage(("[State] #ffffff %s"):format(text), self.m_StateColor.r, self.m_StateColor.g, self.m_StateColor.b, true)
			end
					...
					if string.len(reason) > 2 and string.len(reason) < 50 then
						if target.m_LastWantedsByReason then
							if target.m_LastWantedsByReason[reason] and target.m_LastWantedsByReason[reason] > getTickCount() - 3*60*1000 then -- do not allow wanteds with same reason within three minutes
								player:sendWarning(_("%s has already gotten Wanteds for %s in the last three minutes!", player, target:getName(), reason))
								return
							end
						else
						...
						outputChatBox(_("Crime committed: %s, %s Wanted/s, Reported by: %s", target, reason,amount,player:getName()), target, 255, 255, 0 )
						local msg = ("%s has given %s %d Wanted/s because of %s!"):format(player:getName(),target:getName(),amount, reason)
						player:getFaction():addLog(player, "Wanteds", "gave "..target:getName().." "..amount.." Wanteds for "..reason.."!")
						...
						player:sendError(_("The reason is invalid!", player))
					end
				else
					player:sendError(_("The player is dead!", player))
				end
			end
		else
			player:sendError(_("The number must be between 1 and %s!", player, MAX_WANTED_LEVEL))
		end
	else
		player:sendError(_("You're not on duty!", player))
	end
...
function FactionState:Command_stvo(player, cmd, target, category, amount,...)
	if player:isFactionDuty() and player:getFaction() and player:getFaction():isStateFaction() == true then
		...
		if amount and amount >= 1 and amount <= 6 then
			...
			if isElement(target) then
				if string.len(reason) > 2 and string.len(reason) < 50 then
					...
					if category then
						...
						outputChatBox(_("You have received %d RTR point/s from %s! Total: %d", target, amount, player:getName(), newSTVO), target, 255, 255, 0 )
						outputChatBox(_("Reason: %s", target, reason), target, 255, 255, 0 )

						local msg = ("%s has given %s %d RTR point/s because of %s!"):format(player:getName(),target:getName(),amount, reason)
						player:getFaction():addLog(player, "RTR", "gave "..target:getName().." "..amount.." RTR-Points for "..reason.."!")
						self:sendMessage(msg, 255,0,0)
					else
						player:sendError(_("The Category is invalid!", player))
					end
				else
					player:sendError(_("The Reason is invalid!", player))
				end
			...
			player:sendError(_("The Number must be between 1 and %s!", player, MAX_RTR_REQUEST))
		end
	else
...
function FactionState:Event_giveSTVO(target, category, amount, reason)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			local newSTVO = target:getSTVO(category) + amount
			target:setSTVO(category, newSTVO)
			outputChatBox(_("You have received %d RTR point/s from %s! Total: %d", target, amount, client:getName(), newSTVO), target, 255, 255, 0)
			outputChatBox(_("Reason: %s", target, reason), target, 255, 255, 0 )
			local msg = ("%s has given %s %d RTR point/s because of %s!"):format(client:getName(),target:getName(),amount, reason)
			client:getFaction():addLog(client, "RTR", "gave "..target:getName().." "..amount.." RTR-Points for "..reason.."!")
			self:sendMessage(msg, 255,0,0)
		end
	end
end
...
function FactionState:Event_setSTVO(target, category, amount, reason)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			...
			target:setSTVO(category, newSTVO)
			outputChatBox(_("%s has set your RTR point/s to %d!", target, client:getName(), newSTVO), target, 255, 255, 0 )
			outputChatBox(_("Reason: %s", target, reason), target, 255, 255, 0 )
			local msg = ("%s has set the RTR points of %s to %d! Reason: %s"):format(client:getName(), target:getName(), amount, reason)
			client:getFaction():addLog(client, "RTR", "set the RTR-Points of "..target:getName().." to "..amount.."! Reason: "..reason.."!")
			self:sendMessage(msg, 255,0,0)
		end
	end
end
				...
				if tname then
					...
					if isElement(target) then
						if target == player then
							player:sendError(_("You can't tie yourself up!", player))
							return
						end
						if force == true or (target:getOccupiedVehicle() and target:getOccupiedVehicle() == vehicle) then
							if not target.isGrabbedInVehicle or (force and bool) then
								...
								if not force then
									player:sendInfo(_("You have %s tied up", player, target:getName()))
									target:sendInfo(_("You were tied up by %s", target, player:getName()))
								end
							else
								...
								if not force then
									player:sendInfo(_("You've unleashed %s", player, target:getName()))
									target:sendInfo(_("You were unleashed by %s", target, player:getName()))
								end
							end
						else
							player:sendError(_("The player isn't in your vehicle!", player))
						end
					end
				else
					player:sendError(_("No destination specified! Command: /tie [NAME]!", player))
				end
			else
				player:sendError(_("You're not in a faction vehicle!", player))
			end
...
function FactionState:Event_JailPlayer(player, bail, CUTSCENE, police, force, pFactionBonus, offline, isoCell)
	...
	if not force then
			...
			if policeman:isFactionDuty() then
				if player:getWanteds() > 0 then
					...
					if mon < factionBonus then
						...
						player:transferMoney(self.m_BankAccountServer, mon, "Jail Sentence (bar)", "Faction", "Arrest")
						if remainMoney > bankM then
							player:transferMoney(self.m_BankAccountServer, bankM, "Jail Sentence (Bank)", "Faction", "Arrest")
						else
							player:transferMoney(self.m_BankAccountServer, remainMoney, "Jail Sentence (Bank)", "Faction", "Arrest")
						end
					else
						player:transferMoney(self.m_BankAccountServer, factionBonus, "Jail Sentence (bar)", "Faction", "Arrest")
					end
					...
					PlayerManager:getSingleton():sendShortMessage(_("%s has just been locked up by %s for %d minutes! Penalty: %d$", player, player:getName(), policeman:getName(), jailTime, factionBonus), "Staat")
					StatisticsLogger:getSingleton():addArrestLog(player, wantedLevel, jailTime, policeman, bailcosts)
					policeman:getFaction():addLog(policeman, "Prison", "has incarcerated "..player:getName().." for "..jailTime.." minutes!")
					-- Give Achievements
					...
					policeman:sendError(_("The player is not wanted!", player))
				end
			else
				policeman:sendError(_("You're not on duty!", player))
			end
		end
	else
		local bailcosts = 0
		...
		if mon < factionBonus then
			...
			if remainMoney > bankM then
				player:transferBankMoney(self.m_BankAccountServer, bankM, "Jail Sentence (bank)", "Faction", "Arrest")
			else
				player:transferBankMoney(self.m_BankAccountServer, remainMoney, "Jail Sentence (bank)", "Faction", "Arrest")
			end
...
function FactionState:payArrestBonus(player, bonus)
	self.m_BankAccountServer:transferMoney({player, true}, bonus, "Imprisonment-Premium", "Faction", "Arrest Bonus")
end
		...
		if player.m_Bail and player.m_JailTime then
			if player.m_Bail > 0 then
				...
				if money >= player.m_Bail then
					...
					player:transferBankMoney({FactionManager:getSingleton():getFromId(1), nil, true}, player.m_Bail, "Security-Deposit", "Faction", "Bail")

					player:sendInfo(_("You bought your freedom with the bail money of %s$!", player, player.m_Bail))
					player.m_Bail = 0
					self:freePlayer(player)
				else
					player:sendError(_("You don't have enough money!", player))
				end
			end
		end
	end
			...
			if instanceof(stateVehicle, FactionVehicle) and stateVehicle:getFaction():isStateFaction() then
				if client.vehicleSeat == 0 then
					if not client.m_SpeedCol then
						...
						client:sendInfo("Speed trap is activated!!")
						...
						client:sendInfo("Speed trap is deactivated!")
...
function FactionState:freePlayer(player, prisonBreak)
	if prisonBreak then
		player:sendShortMessage("You've broken out of prison!")
		self:sendShortMessage(player:getName().." has broken out of prison!")
	else
...
function FactionState:Event_FactionRearm()
	if not self:isPlayerInDutyPickup(client) then return client:sendError(_("You're too far away!", client)) end
	if client:isFactionDuty() then
...
function FactionState:Event_toggleDuty(wasted, preferredSkin, dontChangeSkin, player)
	...
	if getPedOccupiedVehicle(client) then
		return client:sendError(_("Get out of the vehicle first!", client))
	end
	local faction = client:getFaction()
	if faction:isStateFaction() then
		if self:isPlayerInDutyPickup(client) or wasted then
			if client:isFactionDuty() then
				...
				client:sendInfo(_("You're no longer on duty!", client))
				...
				Guns:getSingleton():setWeaponInStorage(client, false, false)
				client:sendInfo(_("You are now on duty!", client))
				client:getInventory():removeAllItem("Warnkegel")
		...
		client:sendError(_("You're not in any state faction!", client))
		return false
	end
end
	...
	for index, cop in pairs(colPlayers) do
		if cop:getFaction() and cop:getFaction():isStateFaction() and cop:isFactionDuty() and not cop:isDead() and player ~= cop then
			if player:getInterior() == cop:getInterior() and player:getDimension() == cop:getDimension() then
				...
				player:addOfflineMessage( "You were locked up offline! The jail time is therefore longer!", 1)
				return
			end
		end
	end
...
function FactionState:Event_giveWanteds(target, amount, reason)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			target:giveWanteds(amount)
			outputChatBox(_("Crime committed: %s, %s wanted level/s, Reported by: %s", client, reason, amount, client:getName()), target, 255, 255, 0 )
			local msg = ("%s has given %s %d wanted level/s for %s!"):format(client:getName(), target:getName(), amount, reason)
			faction:addLog(client, "Wanteds", "has given "..target:getName().." "..amount.." wanted level/s! Reason: "..reason)
			PoliceAnnouncements:getSingleton():triggerWantedSound(target, reason)
			self:sendMessage(msg, 255,0,0)
		end
	end
...
function FactionState:Event_clearWanteds(target)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			...
			outputChatBox(_("All your wanted level has been cleared by %s", target, client:getName()), target, 255, 255, 0 )
			local msg = ("%s has cleared all wanteds for %s!"):format(client:getName(), target:getName())
			faction:addLog(client, "Wanteds", "has cleared all wanteds for "..target:getName().."!")
			self:sendMessage(msg, 255,0,0)
		end
	end
end
				...
				if target.isTasered == true and not target:isDead() then
					for seat = 1, getVehicleMaxPassengers(vehicle) do
						if not vehicle:getOccupant(seat) then
							...
							client:sendInfo(_("%s has been pulled into your vehicle!", client, target:getName()))
							target:sendInfo(_("You have been pulled into the vehicle by %s!", target, client:getName()))
							self:Command_tie(client, "tie", target:getName(), true, true)
							return
						end
					end
					client:sendError(_("You have no space in your vehicle!", client))
				else
					client:sendError(_("The player isn't tasered or dead!", client))
				end
			else
				client:sendError(_("You aren't in a faction vehicle!", client))
			end
		else
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			if (target.vehicle or client.vehicle) and client.vehicle ~= target.vehicle then
				client:sendError(_("You can't search the player this way!", target))
				return
			end

			target:sendInfo(_("Government Official %s Searching You!", target, client:getName()))
...
local faction = client:getFaction()
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			if (target.vehicle or client.vehicle) and client.vehicle ~= target.vehicle then
				client:sendError(_("You can't test the player for alcohol in a vehicle!", target))
				return
			end
			client:meChat(true, ("Perform an Alcohol test on %s!"):format(target:getName()))
			target:meChat(true, "blows into the tube of the rapid alcohol tester...")
			setTimer(function(player, target)
				player:sendInfo(_("You have conducted an alcohol test on %s!\nResult: %s Promille", player, target:getName(), target.m_AlcoholLevel))
				target:sendInfo(_("%s has conducted an alcohol test on you!\nResult: %s Promille", target, player:getName(), target.m_AlcoholLevel))
			end, 2200, 1, client, target)
		else
			client:sendError(_("You're not on duty!", client))
		end
	end
...
function FactionState:Event_showLicenses(target)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			QuestionBox:new(target, _("Government Official %s asks you to show your driver's license! Will you show him your driver's license?", client, getPlayerName(client)), "factionStateAcceptShowLicense", "factionStateDeclineShowLicense", client, 20, client, target)
		else
			client:sendError(_("You're not on duty!", client))
		end
	end
...
function FactionState:Event_acceptShowLicense(player, target)
	...
	target:meChat(true, _("nods.", target))
	player:meChat(true, _("looks at %s driver's license.", player, target:getName()))
end

function FactionState:Event_declineShowLicense(player, target)
	target:meChat(true, _("Shakes his head.", target))
end
	...
	if faction and faction:getId() == 3 then
		if client:isFactionDuty() then
			if not PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "faction", "setPaNote") then
				client:sendError(_("You are not authorized to give out GWD-Scores!", client))
				return
			end
			if client == target then
				client:sendError(_("You are not allowed to set yourself a GWD-Score!", client))
				return
			end
			if note > 0 and note <= 100 then
				target:sendInfo(_("%s has given you a GWD-Score of %d!", target, client:getName(), note))
				client:sendInfo(_("You have given %s a GWD-Score of %d!", client, target:getName(), note))
				target:setPaNote(note)
				client:getFaction():addLog(client, "GWD", ("has given %s a GWD-Score of %d!"):format(target:getName(), note))
			else
				client:sendError(_("Invalid GWD-Score!", client))
			end
		else
			client:sendError(_("You're not on duty!", client))
		end
	else
		client:sendError(_("You aren't in the SASF!", client))
	end
end
...
function FactionState:Event_takeDrugs(target)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			local DrugItems = self.ms_IllegalItems
			client:sendMessage(_("You took the following illegal items from %s:", client, target:getName()), 255, 255, 0)
			target:sendMessage(_("%s took the following illegal items from you:", target, client:getName()), 255, 255, 0)
			...
			if not drugsTaken then
				client:sendMessage(_("None", client), 255, 125, 0)
				target:sendMessage(_("None", target), 255, 125, 0)
			end
		else
			client:sendError(_("You're not on duty!", client))
		end
...
function FactionState:Event_takeWeapons(target)
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() then
			client:sendMessage(_("You've disarmed %s!", client, target:getName()), 255, 255, 0)
			target:sendMessage(_("%s has disarmed you!", target, client:getName()), 255, 255, 0)
			for i = 0, 8 do
			...
			if veh:getFaction() and veh:getFaction():isStateFaction() then
				if FACTION_TRUNK_SWAT_ITEMS[itemName] then
					if client.vehicle and not (client.vehicle:getModel() == 427 or client.vehicle:getModel() == 433 or client.vehicle:getModel() == 528) then
						client:sendError(_("This item can only be placed in an Enforcer, FBI Truck or Barracks!", client))
						return
					else
						if not client.vehicle then
							client:sendError(_("No vehicle found!", client))
							return
						end
					end
				end
				...
				client:sendError(_("Invalid vehicle!", client))
			end
		else
			client:sendError(_("You're not on duty!", client))
		end
	else
		client:sendError(_("You need a vehicle to load it!", client))
	end
end
	...
	if faction and faction:isStateFaction() then
		if client:isFactionDuty() and PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "faction", "releaseFromJail") then
			if target and isElement(target) then
				outputChatBox(_("You've been released from prison by %s!", target, client:getName()), target, 255, 255, 0 )
				local msg = ("%s has released %s from prison!"):format(client:getName(), target:getName())
				self:sendMessage(msg, 255,0,0)
				faction:addLog(client, "Prison", "released "..target:getName().." from prison!")
				self:freePlayer(target)
			else
				client:sendError(_("Player not found!", client))
			end
		else
			client:sendError(_("You are not authorised to release a player from prison!", client))
		end
	end
end
		...
		if bugData["element"] and isElement(bugData["element"]) then
			if (player:getPosition() - bugData["element"]:getPosition()).length < range then
				if getTickCount() - self.m_Bugs[id]["lastMessage"] >= 300000 then
					self:sendShortMessage("Bug "..id.." has received new information!\n(Press F4)")
				end
...
function FactionState:Event_attachBug()
	...
	if id then
		...
		client:triggerEvent("receiveBugs", self.m_Bugs)
		self:sendShortMessage(client:getName().." has attached Bug "..id.." to a "..typeName.."!")
	else
		client:sendError(_("All available bugs are active!", client))
	end
end
	...
	if self.m_Bugs[id] then
		if action == "disable" then
			if PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "faction", "disableBug") then
				...
				client:sendError(_("You are not authorised to disable bugs", client))
			end
		elseif action == "clearLog" then
			if PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "faction", "clearBugLog") then
				self.m_Bugs[id]["log"] = {}
				client:sendSuccess(_("You have deleted the log of the bug %d!", client, id))
			else
				client:sendError(_("You are not authorised to delete the bug log", client))
			end
		end
		client:triggerEvent("receiveBugs", self.m_Bugs)
	else
		client:sendError(_("Bug not available!", client))
	end
end

function FactionState:Event_checkBug(element)
	local checkElement = client
	local text = _("Your Body", client)
	local price = 25
	if element then
		checkElement = element
		text = _("Your Vehicle", client)
		price = 50
	end
	if client:getMoney() >= price then
		client:transferMoney(CompanyManager:getSingleton():getFromId(CompanyStaticId.MECHANIC), price, "Bug-Check", "Company", "BugCheck")
		if checkElement:getData("Wanze") == true and checkElement.BugId then
			local id = checkElement.BugId
			self.m_Bugs[id]["element"].BugId = nil
			self.m_Bugs[id]["element"]:setData("Wanze", false, true)
			self.m_Bugs[id] = {}
			self:sendShortMessage("Bug "..id.." was discovered and removed!")
			client:sendShortMessage(_("Oops! I removed a bug from %s!", client, text))
		else
			client:sendShortMessage(_("I found no bug on %s!", client, text))
		end
	else
		client:sendError(_("You don't have enough money! (%d$)", client, price))
	end
end
			...
			if (hitElement:getFaction() and not hitElement:getFaction():isStateFaction()) or not hitElement:getFaction() then
				if not isTimer(self.m_AlertTimer) then
					if not self.m_AreaAlert then
						...
						FactionManager.Map[3]:sendWarning("Unauthorised persons have been spotted in the restricted zone!", "Code Red", false)
					end
				end
...
function FactionState:Event_DeactivateAreaAlarm()
	if PermissionsManager:getSingleton():hasPlayerPermissionsTo(client, "faction", "toggleAreaAlarm") then
		...
		FactionManager.Map[3]:sendShortMessage(("%s has the area alert %s!"):format(client:getName(), self.m_AreaAlarmActivated and "Activated" or "Deactivated"))
	else
		client:sendError(_("You aren't authorised to Deactivate/Activate the area alarm!", client))
	end
end
...
function FactionState:Event_dragFromVehicle(target)
	...
	if veh and isElement(veh) and veh.type == "vehicle" then
		...
		target:sendShortMessage(_("You were pulled out of the vehicle!", target))
	end
end
...
function FactionState:Event_removeIllegalTunings(vehicle)
	...
	if client:getFaction() and client:getFaction():isStateFaction() then
		--if vehicle:hasRadarDetector() then
			...
			client:sendSuccess(_("Illegal tunings removed!", client))
			...
			if getElementData(vehicle, "OwnerType") == "player" and DatabasePlayer.getFromId(vehicle:getOwner()) then
				...
				vehicleOwner:sendInfo(_("%s %s has removed all illegal tuning from your vehicle %s (%d)", vehicleOwner, rankName, client:getName(), getVehicleNameFromModel(vehicle:getModel()), vehicle:getId()))
			elseif getElementData(vehicle, "OwnerType") == "group" then
				for i, player in pairs(vehicle:getGroup():getOnlinePlayers()) do
					player:sendInfo(_("%s %s has removed all illegal tuning from your vehicle %s (%d)", player, rankName, client:getName(), getVehicleNameFromModel(vehicle:getModel()), vehicle:getId()))
				end
			end
