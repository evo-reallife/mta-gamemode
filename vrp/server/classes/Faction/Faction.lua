...
function Faction:setSetting(category, key, value, responsiblePlayer)
	...
	if allowed then
		...
		responsiblePlayer:sendError(_("You are not authorized to change the %s settings!", responsiblePlayer, category))
	end
end
	...
	if player:isFactionDuty() then
		if self.m_Skins[skinId] then
			...
			if minRank <= playerRank then
				...
				player:sendWarning(_("Your selected clothing is only available from rank %s, you have been given a different one.", player, minRank))
		...
		player:sendError(_("You are Off Duty Faction!", player))
	end
end
...
function Faction:removePlayer(playerId)
	...
	if player then
		...
		player.m_KevlarShotsCount = nil
		player:setData("Faction:InSpecialDuty", nil, true)
		player:sendShortMessage(_("You've been dismissed from your faction!", player))
		self:sendShortMessage(_("%s has left your faction!", player, player:getName()))
		player:reloadBlips()
...
function Faction:invitePlayer(player)
  client:sendShortMessage(("You have successfully invited %s to the faction."):format(getPlayerName(player)))
...
function Faction:__giveMoney(amount, reason, silent)
	StatisticsLogger:getSingleton():addMoneyLog("faction", self, amount, reason or "Unknown")
...
function Faction:sendChatMessage(sourcePlayer, message)
	if not getElementData(sourcePlayer, "FactionChatEnabled") then return sourcePlayer:sendError(_("You have deactivated the fraction chat!", sourcePlayer)) end
...
function Faction:sendBndChatMessage(sourcePlayer, message, alliance)
	if not getElementData(sourcePlayer, "AllianceChatEnabled") then return sourcePlayer:sendError(_("You have deactivated the alliance chat!", sourcePlayer)) end
	local playerId = sourcePlayer:getId()
	local receivedPlayers = {}
	local r,g,b = 20, 140, 0
	local text = ("[Alliance] %s: %s"):format(getPlayerName(sourcePlayer), message)
	for k, player in ipairs(self:getOnlinePlayers()) do
...
function Faction:respawnVehicles(player)
	...
	if not self:isRespawnPossible() and not isAdmin then
		return self:sendShortMessage("Vehicles can only respawn every 15 minutes!")
	end

	if isAdmin then
		self:sendShortMessage("An admin has respawned your faction vehicles!")
		player:sendShortMessage("You have respawned the faction vehicles!")
	end
	...
	self:sendShortMessage(("%s/%s Vehicles have been respawned!"):format(vehicles-fails, vehicles))
end
	...
	for k, player in ipairs(self:getOnlinePlayers()) do
		if not player:getPhonePartner() then
			if player ~= caller then
				local color = {factionColors[self.m_Id].r, factionColors[self.m_Id].g, factionColors[self.m_Id].b}
				triggerClientEvent(player, "callIncomingSM", resourceRoot, caller, false, ("%s will call you."):format(caller:getName()), ("Incoming call - %s"):format(self:getShortName()), color, "faction")
			end
		end
	end
end
	...
	if player and caller then
		if instanceof(caller, Player) and instanceof(player, Player) then -- check if we can call methods from the Player-class
			if player.m_PhoneOn == false then
				player:sendError(_("Your phone is turned off!", player))
				return
			end
			if player:getPhonePartner() then
				player:sendError(_("You're already on the phone!", player))
				return
			end
			caller:triggerEvent("callAnswer", player, voiceCall)
			player:triggerEvent("callAnswer", caller, voiceCall)
			self:addLog(player, "Anrufe", ("has a phone call with %s!"):format(caller:getName()))
	...
	addEventHandler("onElementClicked", self.m_Safe, function(button, state, player)
		if button == "left" and state == "down" then
			if player:getFaction() and player:getFaction() == self or (player:getFaction() and player:getFaction():isStateFaction() and self:isStateFaction()) then
				...
				player:sendError(_("You're not in the right group", player))
			end
		end
	end)
...
function Faction:changeDiplomacy(targetFaction, diplomacy, player)
	...
	for index, data in pairs(self.m_Diplomacy) do
		...
		if factionId == targetFaction:getId() then
			...
			if player then
				self:sendShortMessage(("%s has changed the diplomatic status with the %s to '%s'!"):format(player:getName(), targetFaction:getName(), FACTION_DIPLOMACY[diplomacy]))
			end
...
function Faction:createDiplomacyRequest(sourceFaction, targetFaction, diplomacy, player)
	...
	if player and sourceFaction == self then
		self:sendShortMessage(("%s has sent a %s-request to the group %s!"):format(player:getName(), targetFaction:getName(), FACTION_DIPLOMACY[diplomacy]))
	elseif player and targetFaction == self then
		targetFaction:sendShortMessage(("The %s group has sent you a %s request!"):format(sourceFaction:getName(), FACTION_DIPLOMACY[diplomacy]))
	end
end
...
function Faction:updateEquipmentPermissions(player, update)
	...
	self:sendShortMessage(("Equipment ranks have been updated by %s!"):format(player:getName()))
	self:addLog(player, "Equipment", "has updated the accesses!")
end
...
function Faction:storageWeapons(player, weapons)
					...
					if THROWABLE_WEAPONS[weaponId] then -- grenade etc
						...
						elseif magazines > 0 then
							...
							if weaponsToMax > 0 then
								...
								player:sendError(_("There is no room for %s %s in the depot! Only %s have been stored.", player, magazines, WEAPON_NAMES[weaponId], weaponsToMax))
							else
								player:sendError(_("There is no room for a %s in the depot!", player, WEAPON_NAMES[weaponId]))
							end
						end
					else
						if depotWeapons+1 <= depotMaxWeapons then
							...
							elseif magazines > 0 then
								...
								if magsToMax > 0 then	
									...
									player:sendError(_("There is no space in the depot for %s %s magazine/s! Only %s magazines have been stored.", player, magazines, WEAPON_NAMES[weaponId], magsToMax))
								end
							end
						else
							player:sendError(_("There is no room for a %s in the depot!", player, WEAPON_NAMES[weaponId]))
	...
	local textForPlayer = "You have placed the following weapons in storage:"
