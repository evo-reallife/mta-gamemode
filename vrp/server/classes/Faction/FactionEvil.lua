...
function FactionEvil:createInterior(Id, faction)
	...
	ElementInfo:new(self.m_WeaponPed[Id], "Weapons Storage")
	...
	ElementInfo:new(self.m_ItemDepot[Id], "Item Storage")
	...
	ElementInfo:new(self.m_EquipmentDepot[Id], "Equipment Storage")
...
function FactionEvil:onWeaponPedClicked(button, state, player)
	if button == "left" and state == "up" then
		if player:getFaction() and (player:getFaction() == source.Faction or source.Faction:checkAlliancePermission(player:getFaction(), "weapons")) then
			...
			player:sendError(_("This arms seller does not deliver to your faction!", player))
		end
	end
end
...
function FactionEvil:putOrderInDepot(player, box)
	...
	if type == "Waffe" or self:isSpecialProduct(product) then
		if not self:isSpecialProduct(product) then
			if id then
				...
				player:getFaction():sendShortMessage(("%s has put %s weapon/s [ %s ] into storage!"):format(player:getName(), insertWeapomAmount, product))
				player:getFaction():sendShortMessage(("%s has put %s ammunition [ %s ] into storage!"):format(player:getName(), insertAmmoAmount, product))
				player:getFaction():addLog(player, "Lager", ("%s has put %s weapon/s [ %s ] into storage!"):format(player:getName(), insertWeapomAmount, product))
			end
		else
			if id then
				if depotInfo[id]["Waffe"] >= depot.m_Weapons[id]["Waffe"] + amount then
				...
				player:getFaction():sendShortMessage(("%s has put %s special weapon/s [ %s ] into storage!"):format(player:getName(), insertWeapomAmount, product))
				player:getFaction():sendShortMessage(("%s has put %s special ammunition [ %s ] into storage!"):format(player:getName(), insertAmmoAmount, product))
				player:getFaction():addLog(player, "Lager", ("%s has put %s special weapon/s [ %s ] into storage!"):format(player:getName(), insertAmmoAmount, product))
			end
		end
	elseif type == "Munition" then
		if id then
			...
			depot:addMagazineD(id,insertAmmoAmount)
			player:getFaction():sendShortMessage(("%s has put %s ammunition [ %s ] into storage!"):format(player:getName(), insertAmmoAmount, product))
			player:getFaction():addLog(player, "Lager", ("%s has put %s ammunition [ %s ] into storage!"):format(player:getName(), insertAmmoAmount, product)) 
		end
	else
		depot:addEquipment(player, product, amount, true)
		player:getFaction():sendShortMessage(("%s has put %s pieces of %s into storage!"):format(player:getName(), amount, product))
		player:getFaction():addLog(player, "Lager", ("%s has put %s pieces of %s into storage!"):format(player:getName(), amount, product))
	end
...
function FactionEvil:loadYakGates(factionId)
	...
	elevator:addStation("Roof", Vector3(1418.78, -1329.92, 23.99))
	elevator:addStation("Backyard", Vector3(1423.35, -1356.26, 13.57))
	elevator:addStation("Underground Garage", Vector3(1413.57, -1355.19, 8.93))
...
function FactionEvil:loadTriadGates(factionId)
	...
	elevator:addStation("Roof - Heliports", Vector3(1941.15, 988.72, 52.74), 0)
end
		...
		if target and isElement(target) and target:isLoggedIn() then
			if not target:isFactionDuty() and not target:isCompanyDuty() then
				if client.vehicle then
					client:sendError(_("You can't rob someone from a vehicle!", client))
					return
				end
				...
				if target:getPublicSync("supportMode") then
					client:sendError(_("You can't rob active supporters!", client))
					return
				end

				if target:getInterior() > 0 then
					client:sendError(_("You can only rob people outdoors", client))
					return
				end

				-- Players Under who have 10 hours can't be raided

				if math.floor(target:getPlayTime()/60) < 10 then
					client:sendError(_("Players under 10 game hours may not be robbed!", client))
					return
				end

				if target:getMoney() > 0 then
					local targetName = target:getName()
					-- The Plater has been robbed within the last 2 hours
					if self.m_Raids[targetName] and not timestampCoolDown(self.m_Raids[targetName], 2*60*60) then
						client:sendError(_("This player has already been robbed within the last 2 hours!", client))
						return
					end
					target:sendMessage(_("You're being robbed by %s (%s)!", target, client:getName(), client:getFaction():getShortName()), 255, 0, 0)
					target:sendMessage(_("Run away or stay until the robbery is over!", target), 255, 0, 0)
					client:meChat(true, _("rob's %s!", client, targetName))

					target:triggerEvent("CountdownStop",  15, "Robbery in")
					target:triggerEvent("Countdown", 15, "Robbery in")
					client:triggerEvent("Countdown", 15, "Robbery in")
					client:triggerEvent("factionEvilStartRaid", target)
					self.m_Raids[targetName] = getRealTime().timestamp
				else
					client:sendError(_("The player has no money with him!", client))
				end
			else
				client:sendError(_("You can't rob players on duty!", client))
			end
		end
	end
end
...
function FactionEvil:Event_SuccessRaid(target)
	...
	if money > 0 then
		client:meChat(true, "Successfully robs " .. target:getName() .. "!")
		target:meChat(true, _("was successfully raided by %s!", target, client:getName()))
		target:transferMoney(client, money, "Überfall", "Faction", "Robbery")
		client:triggerEvent("CountdownStop", "Robbery in", 15)
		target:triggerEvent("CountdownStop", "Robbery in", 15)
...
function FactionEvil:Event_FailedRaid(target)
	target:sendSuccess(_("You've escaped the robbery!", target))
	client:sendWarning(_("The player has escaped the robbery!", client))
	target:meChat(true, _("has escaped from the %s rob!", target, client:getName()))
...
function FactionEvil:setPlayerDuty(player, state, wastedOrNotOnMarker, preferredSkin, dontChangeSkin)
	...
	if not state and player:isFactionDuty() then
		...
		player:sendInfo(_("You're now in civilian clothes!", player))
	...
	elseif state and not player:isFactionDuty() then
		...
		player:sendInfo(_("You are now marked as a faction member!", player))
		if not wastedOrNotOnMarker then faction:updateDutyGUI(player) end
	end
end
...
function FactionEvil:Event_toggleDuty(wasted, preferredSkin, dontChangeSkin, player)
	...
	if faction:isEvilFaction() then
		...
		client:sendError(_("You're not in a Gang / Mafia!", client))
		return false
	end
end
