	...
	if getElementType(hitElement) == "player" and matchingDimension then
		...
		if vehicle then
			...
			if table.size(getVehicleOccupants(vehicle)) > 1 then
				hitElement:sendError(_("You can only drive into your garage without a passenger!", hitElement))
				return
			end
			if not instanceof(vehicle, Vehicle) or not vehicle:isPermanent() then
				hitElement:sendError(_("Non-permanent vehicles cannot be parked in the garage!", hitElement))
				return
			end
			if instanceof(vehicle, CompanyVehicle) then
				hitElement:sendError(_("You can't park company cars in the garage!", hitElement))
				return
			end
			if instanceof(vehicle, FactionVehicle) then
				hitElement:sendError(_("You cannot park faction vehicles in the garage!", hitElement))
				return
			end
			if instanceof(vehicle, GroupVehicle) then
				hitElement:sendError(_("You cannot park gang vehicles in the garage!", hitElement))
				return
			end
			if not instanceof(vehicle, Vehicle) or vehicle:getOwner() ~= hitElement:getId() then
				hitElement:sendError(_("You can only park your own vehicles in the garage!", hitElement))
				return
			end
    	end

	    if hitElement:getGarageType() == 0 then
	      hitElement:sendError(_("You don't own a garage!", hitElement))
	      return
	    end
			...
			function (source)
				...
				if vehicle then
					if #session:getSlots() == self:getMaxSlots(hitElement:getGarageType()) then
						hitElement:sendError(_("This garage has no space for another vehicle! Get out!", hitElement))
						return
					end
				end
			end, 1000, 1, source
...
function VehicleGarages.initalizeAll()
	VehicleGarages:new(
		{ -- Eingänge -- Ausgänge
			...
			{enter = {-2199.5, -2350.6, 29.9}, exit = {-2194.00, -2355.07, 30.25, 230}};
