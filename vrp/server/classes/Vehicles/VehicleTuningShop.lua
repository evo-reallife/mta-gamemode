...
function VehicleTuningShop:constructor()
    ...
    for garageId, info in pairs(self.m_GarageInfo) do
		...
		local blipText = "Tuning Garage"

		if info[4] == "AirportPainter" then
			blipText = blipText.." (Airplanes)"
		elseif info[4] == "BoatsPainter" then
			blipText = blipText.." (Boats)"
		end
...
function VehicleTuningShop:openForAdmin(admin, vehicle)
    ...
    self:openFor(admin, vehicle, 1, type, true)
        ...
        admin:sendError(_("You must remain seated in the vehicle!", admin))
    end
end
    ...
    if getElementType(hitElement) == "player" and matchingDimension then
        ...
        if instanceof(vehicle, CompanyVehicle) then
          if not vehicle:canBeModified() then
              hitElement:sendError(_("This company car must not be tuned!", hitElement))
              return
          end
		elseif instanceof(vehicle, FactionVehicle) then
          if not vehicle:canBeModified() then
              hitElement:sendError(_("This faction car must not be tuned!", hitElement))
              return
          end
        elseif instanceof(vehicle, GroupVehicle) then
            if vehicle:getGroup() ~= hitElement:getGroup() then
                hitElement:sendError(_("You can't tune this vehicle!", hitElement))
                return
            end
            if not vehicle:canBeModified()  then
                hitElement:sendError(_("Your leader must activate the tuning of vehicles! In the company/gang menu under Leader item!", hitElement))
                return
            end
            if not PermissionsManager:getSingleton():hasPlayerPermissionsTo(hitElement, "group", "editVehicleTuning") then
                hitElement:sendError(_("You are not authorized to tune group vehicles!", hitElement))
                return
            end
        elseif vehicle:isPermanent() then
            if vehicle:getOwner() ~= hitElement:getId() then
                hitElement:sendError(_("You can only tune your own vehicles!", hitElement))
                return
            end
        else
			hitElement:sendError(_("You can't tune this vehicle!", hitElement))
        ...
        if vehicle:getOccupantsCount(true) > 1 then 
            hitElement:sendError(_("Let your passengers get off first!", hitElement))
            return
        end
		...
		elseif vehicleType == VehicleType.Automobile or vehicleType == VehicleType.Bike then
            ...
            hitElement:sendError(_("You can't enter the tuning workshop with this type of vehicle!", hitElement))
        end
    end
end
...
function VehicleTuningShop:Event_vehicleUpgradesBuy(cartContent)
    ...
    client:sendSuccess(_("Upgrades Purchased!", client))
