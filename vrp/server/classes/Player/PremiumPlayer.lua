		...
		if not freeVIP then
			setTimer(function()
				self.m_Player:sendShortMessage(_([[
					Your premium account is valid
					until %s
				]], self.m_Player, getOpticalTimestamp(self.m_PremiumUntil)), _("Premium", self.m_Player), {50, 200, 255})
			end, 1500, 1)
		else 
			setTimer(function()
				self.m_Player:sendShortMessage(_("You receive free Premium based on your position in the team.", self.m_Player), _("Premium", self.m_Player), {50, 200, 255})
			end, 1500, 1)
		end
	end
...
function PremiumPlayer:openVehicleList()
	...
	if row and #row > 0 then
		...
		self.m_Player:sendError(_("No vehicles ready for collection!", self.m_Player))
	end
end
	...
	if row and row.ID then
		if #self.m_Player:getVehicles() < math.floor(MAX_VEHICLES_PER_LEVEL*self.m_Player:getVehicleLevel()) then
			...
			if vehicle then
				...
				self.m_Player:sendMessage(_("Error creating the vehicle. Please notify an admin!", self.m_Player), 255, 0, 0)
			end
		else
			self.m_Player:sendError(_("Maximum vehicle slot reached!", self.m_Player))
		end
	else
		self.m_Player:sendError(_("Invalid vehicle!", self.m_Player))
	end
end
