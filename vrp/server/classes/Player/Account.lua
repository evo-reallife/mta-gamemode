...
function Account.login(player, username, password, pwhash, enableAutologin)
	...
	if not username:match("^[a-zA-Z0-9_.%[%]]*$") then
		player:triggerEvent("loginfailed", "Invalid nickname. Your name may only use alphanumeric.")
		return false
	end
	...
	if not Account.FORUM_LOGIN then
		...
		if not row then
			player:triggerEvent("loginfailed", "This account does not exist")
			return
		end

		local passwordHash = sha256(row.Salt..password)
		if passwordHash ~= row.Password then
			player:triggerEvent("loginfailed", "Incorrect password")
			return
		end
	...
	elseif resData and resData.status and (resData.status == 400 or resData.status == 412) then
		player:triggerEvent("loginfailed", "Incorrect name or password") -- Error: Invalid username or password2
		return false
	else
...
function Account.loginSuccess(player, Id, Username, ForumId, RegisterDate, TeamspeakId, pwhash)
		...
		if #MultiAccount.getAccountsBySerial(player:getSerial()) > 1 then
			if not MultiAccount.isAccountLinkedToSerial(Id, player:getSerial()) then
				if not MultiAccount.allowedToCreateAnMultiAccount(player:getSerial()) then
					player:triggerEvent("loginfailed", "Your serial is used for multiple accounts. This can happen if someone logs in to your PC with other account data. Please contact us in the forum (forum.evo-reallife.de) under 'administrative requests' to solve the problem.")
					return false
				else
	...
	if not Warn.checkWarn(player, Id, true) then
		...
		if player and isElement(player) then player:triggerEvent("loginfailed", "You have been banned for 3 warnings!") end
		return false
	end

	if not Ban.checkBan(player, Id, true) then
		-- Todo Maybe it´s more beautiful not kicking player directly only display a more information error
		if player and isElement(player) then player:triggerEvent("loginfailed", "You've been banned!") end
		return false
	end
	...
	if not player.loadCharacter then
		outputDebugString( ("Account.loginSuccess: Player-Element %s, geerbt von Player-Klasse: %s"):format(inspect(player), tostring(instanceof(player, Player))) )
		player:kick("Error logging in, please contact support!")
		return
	end
...
function Account.register(player, username, password, email)
	...
	if not Account.REGISTRATION_ACTIVATED then
		player:triggerEvent("registerfailed", _("Registration is temporarily disabled. Please try again later.", player))
		return false
	end
	...
	if not username:match("^[a-zA-Z0-9_.]*$") or #username < 3 or #username > 22 then
		player:triggerEvent("registerfailed", _("Invalid nickname. Your name may only use alphanumeric characters and the underscore (_).", player))
		return false
	end

	if #password < 5 then
		player:triggerEvent("registerfailed", _("Password too short! Min. 5 characters!", player))
		return false
	end

	-- Validate email
	if not email:match("^[%w._-]+@[%w._-]+%.%w+$") or #email > 50 then
		player:triggerEvent("registerfailed", _("Invalid E-mail", player))
		return false
	end
	...
	if MULTIACCOUNT_CHECK then
		if MultiAccount.isSerialUsed(player:getSerial()) then
			if not MultiAccount.allowedToCreateAnMultiAccount(player:getSerial()) then
				player:triggerEvent("registerfailed", _("You already have an account!", player))
				return false
			end
		end
	end
	...
	if not Account.FORUM_LOGIN then
		if sql:queryFetchSingle("SELECT Name FROM ??_account WHERE Name = ?", sql:getPrefix(), username) then
			player:triggerEvent("registerfailed", _("Username is already in use!", player))
			return false
		end

		if sql:queryFetchSingle("SELECT Name FROM ??_account WHERE EMail = ?", sql:getPrefix(), email) then
			player:triggerEvent("registerfailed", _("E-mail is already in use!", player))
			return false
		end
	...
	if data and data.status and data.status == 200 then
		...
		if data and data.message then
			...
			elseif data.message == "username is not invalid" then
				player:triggerEvent("registerfailed", _("Username contains invalid characters", player))
			elseif data.message == "email is not notUnique" then
				player:triggerEvent("registerfailed", _("This E-mail is already in use", player))
			elseif data.message == "email is not invalid" then
				player:triggerEvent("registerfailed", _("This E-mail is not valid", player))
			else
				player:triggerEvent("loginfailed", "Error: Forum-Account could not be created")
			end
		else
			player:triggerEvent("loginfailed", "Error: Forum-Account could not be created")
		end
