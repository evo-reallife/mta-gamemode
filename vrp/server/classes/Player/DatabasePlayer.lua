...
function DatabasePlayer:setAlcoholLevel(level, oldLevel)
	...
	if self:isActive() then
		...
		elseif level < 0 then
			...
			setPedWalkingStyle(self, self:getWalkingstyle())
		elseif level  >= 2 then
		...
		if level >= MAX_ALCOHOL_LEVEL then
			self:sendShortMessage(_("You've been hospitalized for alcohol poisoning!", self))
		...
		if oldLevel then
			...
			self:sendShortMessage(_("Current alcohol content: %s ‰s (%s)", self, tostring(self.m_AlcoholLevel), tostring(diff > 0 and "+"..diff or diff)), "Alkoholgehalt")
		end
...
function DatabasePlayer:setNewNick(admin, newNick)
	if not self:getId() or self:getId() <= 0 then
		admin:sendError(_("Id not found!", admin))
		return false
	end

	if not newNick:match("^[a-zA-Z0-9_.%[%]]*$") or #newNick < 3 or #newNick > 22 then
		admin:sendError(_("Invalid nickname!", admin))
		return false
	end
	...
	if data and data.status and data.status == 200 then
		...
		if data and data.status then
			admin:sendError(_("Nickname already taken!", admin))
		else
			admin:sendError(_("Error: There was a problem with the interface", admin))
		end
		return false
	end
	...
	if self:isActive() then
		...
		self:sendMessage(_("%s has changed your nickname from %s to %s!", self, admin:getName(), oldNick, newNick), 255, 0, 0)
	end
...
function DatabasePlayer:setSTVO(category, stvo)
	if category then
		...
		if stvo >= 20 then
			...
			if self:isActive() then
				self:sendMessage(_("You have 20 STVO points! Your driver's license has been revoked!", self), 255, 0, 0)
			else
				self:addOfflineMessage(_"You had 20 STVO points and your driver's license was revoked!", 1)
			end
