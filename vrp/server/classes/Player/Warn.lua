...
function Warn.addWarn(who, author, reason, duration)
	...
	if Warn.getAmount(who) >= 3 then
		...
		if player then
			kickPlayer(player, _("You have been banned for 3 warnings!", player))
		end
	end
end
...
function Warn.checkWarn(player, id, doNotSave)
	...
	if Warn.getAmount(id) >= 3 then
		...
		if row then
			...
			kickPlayer(player, _("You have 3 warnings! The next one expires in %d days!", player, days+1))
			return false
		end
		return true
	elseif Warn.getAmount(id) > 0 then
		outputChatBox(_("Be careful, you already have %d warning/s!", player, Warn.getAmount(id)),player, 255,0,0)
	end
	return true
end
