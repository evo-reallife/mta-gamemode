...
function Ban.addBan(who, author, reason, duration, adminName)
	...
	if player then
		...
		if duration > 0 then
			reasonstr = ("+Timeban: %s from %s (reason: %s)"):format( string.duration(duration), authorName, reason)
		else
			reasonstr = ("+Permanent ban of %s (reason: %s)"):format(authorName, reason)
		end
...
function Ban.checkSerial(serial, player, id, cancel, doNotSave)
	...
	if row then
		...
		if duration == 0 then
			reasonstr = ("You have been permanently banned (Reason: %s)"):format(row.reason)
		...
		elseif duration > 0 then
			reasonstr = ("You are still %s banned! (Reason: %s)"):format(string.duration(duration - getRealTime().timestamp), row.reason)
		end
