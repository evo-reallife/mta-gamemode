...
function Player:setJailBail( bail )
	if bail > 0 then
		self:sendMessage(_("Prison: You can buy your freedom for %d$ with /bail!", self, bail), 255, 0, 0)
	end
	self.m_Bail = bail
end
...
function Player:loadCharacterInfo()
	...
	if self.m_OfflineMessages then
		for key, msg in ipairs( self.m_OfflineMessages ) do
			self:sendShortMessage(msg[1], "Offline Message", nil, -1)
		end
	end

	if row.TakeWeaponsOnLogin == 1 then
		takeAllWeapons(self)
		self:sendShortMessage("Your weapons were taken away because you were not logged out properly!")
...
function Player:buckleSeatBelt(vehicle)
	...
	if self.vehicle then
		self:sendShortMessage(_("You have %s your seatbelt!", self, self.m_SeatBelt and "buckled" or "unbuckled"))
	end
end
			...
			if SpawnLocationProperty then
				...
				if vehicle and vehicle:getPositionType() == VehiclePositionType.World then
					if vehicle:getSpeed() == 0 then
						...
						self:sendWarning("Spawning on vehicle not possible, vehicle is currently in use")
					end
				else
					self:sendWarning("It is not possible to spawn on the vehicle, the vehicle is at the tow yard or is no longer available")
				end
			end
		elseif self.m_SpawnLocation == SPAWN_LOCATIONS.HOUSE then
			if SpawnLocationProperty then
				...
				if house and house:isValidToEnter(self) then
					...
					self:sendWarning("Spawning in the house is not possible, you no longer have access to the house")
				end
			end
...
function Player:takeEquipment(noOutput)
	...
	if count > 0 then
		if not noOutput then
			self:sendShortMessage(("You have had %i items taken from your equipment!"):format(count), "Medical Center - Los Santos")
		end
	end
end
...
function Player.staticFactionAllianceChatHandler(self, command, ...)
	if self.m_Faction then
		...
		if bndFaction then
			...
			self:sendError(_("Your alliance has no alliance!", self))
		end
	end
end
...
function Player:setXP(xp)
	...
	if self:getLevel() > oldLevel then
		...
		self:sendInfo(_("You have advanced to level %d", self, self:getLevel()))
	end
end
	...
	for player, tick in pairs(self.m_LastDamagedBy) do
		if player and isValidElement(player, "player") then
			if now - tick < 60*3 then
				...
				player:sendShortMessage(_("The player %s has healed himself! (Time: %s Minutes)", player, self:getName(), secondsMinutes), "Combat healing!", nil, -1)
...
function Player:payDay()
	...
	if self:getFaction() then
		...
		if income_faction > 0 then
			self:getFaction():transferMoney({self, true, true}, income_faction, ("Salary for %s"):format(self:getName()), "Faction", "Loan", {silent = true})
	...
	if self:getCompany() then
		...
		if income_company > 0 then
			...
			self:getCompany():transferMoney({self, true, true}, income_company, ("Salary for %s"):format(self:getName()), "Company", "Loan", {silent = true})
	...
	if self:getGroup() then
		...
		if income_group > 0 then
			...
			self:getGroup():transferMoney({self, true, true}, income_group, ("Salary for %s"):format(self:getName()), "Group", "Loan", {silent = true})
	...
	if EVENT_CHRISTMAS then
		income = income + 200
		BankServer.get("event.christmas"):transferMoney({self, true, true}, 200, "Christmas-bonus", "Event", "ChristmasBonus", {silent = true})
		self:addPaydayText("income", _("Christmas bonus", self), 200)
		...
		if self:getFaction() and not self:getFaction():isRescueFaction() and ChristmasTruckManager:getSingleton():getPresentsCount(self:getFaction()) then
			...
			BankServer.get("event.christmas"):transferMoney({self, true, true}, presentMoney, "Christmas tree bonus", "Event", "ChristmasBonus", {silent = true})
			self:addPaydayText("income", _("Christmas tree bonus", self), presentMoney)
		end
	end
	...
	if income_interest > 0 then
		...
		BankServer.get("server.bank"):transferMoney({self, true, true}, income_interest, "Bank interest", "Bank", "Interest", {silent = true})
		self:addPaydayText("income", _("Bank interest", self), income_interest)
	...
	if self:getPlayTime() <= PAYDAY_NOOB_BONUS_MAX_PLAYTIME * 60 then
		...
		BankServer.get("server.bank"):transferMoney({self, true, true}, PAYDAY_NOOB_BONUS, "Welcome-bonus", "Gameplay", "NoobBonus", {silent = true})
		self:addPaydayText("income", _("Welcome-bonus", self), PAYDAY_NOOB_BONUS)
	end
	...
	if outgoing_vehicles > 0 then
		self:addPaydayText("outgoing", _("Vehicle tax", self), outgoing_vehicles)
		self:transferBankMoney({BankServer.get("server.vehicle_tax"), nil, nil, true}, outgoing_vehicles, _("Vehicle tax", self), "Vehicle", "Tax", {silent = true, allowNegative = true})
		...
		for index, house in pairs(houses) do
			...
			if rent > 0 then
				if self:getBankMoney() - rent >= 0 then
					...
					self:transferBankMoney({house.m_BankAccount, nil, nil, true}, rent, _("Rent to %s from %s", self, Account.getNameFromId(house:getOwner()), self:getName()), "House", "Rent", {silent = true})
					self:addPaydayText("outgoing", _("Rent to %s", self, Account.getNameFromId(house:getOwner())), rent)
				else
					self:addPaydayText("info", _("You couldn't pay the rent on %s's house.", self, Account.getNameFromId(house:getOwner())))
	...
	if self:getWanteds() > 0 then
		self:addPaydayText("info", _("You have been issued a Wanted!", self))
		self:takeWanteds(1)
	end

	self:givePoints(points_total, "Payday", true, true)
	self:addPaydayText("info", _("You have received %s points.", self, points_total))

	if EVENT_EASTER then
		self:addPaydayText("info", _("You got 5 Easter eggs!", self))
		self:getInventory():giveItem("Osterei", 5)
	end

	if EVENT_CHRISTMAS then
		self:addPaydayText("info", _("You got 3 candy canes!", self))
		self:getInventory():giveItem("Zuckerstange", 3)

		if self:getFaction() and not self:getFaction():isRescueFaction() and ChristmasTruckManager:getSingleton():getPresentsCount(self:getFaction()) then
			local presentCount = ChristmasTruckManager:getSingleton():getPresentsCount(self:getFaction())
			if presentCount >= ChristmasTruckManager.MaxPresents then
				self:addPaydayText("info", _("You got %s extra candy canes (full tree)!", self, ChristmasTruckManager.SugarcaneBonusIfFull))
	...
	if EVENT_HALLOWEEN then
		self:addPaydayText("info", _("You got 5 sweets!", self))
...
function Player:givePoints(p, reason, bNoSound, silent) -- Overriden
	...
	if p ~= 0 and not silent then
		self:sendShortMessage(("%s Points%s"):format("+"..p, reason ~= nil and " - "..reason or ""), "Spielfortschritt", {0, 94, 255}, 3000)
	end
end
	...
	if PlayerAttachObjects[model] then
		if not self:getPlayerAttachedObject() then
			...
			if settings.blockVehicle and (self.vehicle or (getElementAttachedTo(self) and getElementType(getElementAttachedTo(self)) == "vehicle")) then
				self:sendError(_("You cannot be in vehicles with this object!", self))
				return false
			end
			if settings.blockFlyingVehicles then
				...
				if veh and veh:isAirVehicle() or self:getPublicSync("isDoingHelicopterDriveby") then
					self:sendError(_("You cannot be in flying devices with this object!", self))
					return false
				end
			end
			...
			if settings.placeDown then
				self:sendShortMessage(_("Press 'n' to drop the %s!", self, settings["name"]))
			...
			self:sendError(_("You already have an object with you!", self))
		end
	else
...
function Player:attachToVehicle(forceDetach)
	...
	if self.contactElement:getVehicleType() == VehicleType.Boat or VEHICLE_PICKUP[self.contactElement:getModel()] then
		if self.contactElement:getSpeed() < 20 then
			...
			self:sendShortMessage(_("Press 'X' to stop holding on to the vehicle.", self))
		else
			self:sendWarning(_("This vehicle is too fast to hold on to!", self))
		end
	end
end
...
function Player:endPrison()
	if isElement(self) then
		...
		self:sendInfo(_("You've been released from prison! Behave better now!", self))
	end
