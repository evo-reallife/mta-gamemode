...
function PhoneInteraction:callStart(player, voiceEnabled, showSkin)
	...
	if not player:isPhoneEnabled() then
		client:sendError(_("Busy... The player is currently unavailable!", client, player.name))
	...
	if player and isElement(player) and player.getPhonePartner then
		if player:getPhonePartner() then
			...
			if partner and isElement(partner) then
				...
				partner:sendWarning(_("Crack... The call was canceled!", partner))
	...
	for index, instance in pairs(PhoneNumber.Map) do
		if instance:getNumber() == number then
			if instance:getOwner(instance) ~= client then
				...
				client:sendError(_("You can't call yourself!", client))
			end
		end
	end
end
...
function PhoneInteraction:callSendLocation()
	if client:getPhonePartner() then
		...
		if self.m_LocationBlips[client] then
			...
			partner:sendMessage(_("[CELL PHONE] %s has updated its position.", partner, client:getName()), 255, 0, 0)
			client:sendMessage(_("[CELL PHONE] You have updated your position.", client, partner:getName()), 255, 0, 0)
		else
			partner:sendMessage(_("[CELL PHONE] %s has told you his position.", partner, client:getName()), 255, 0, 0)
			client:sendMessage(_("[CELL PHONE] You have told %s your position.", client, partner:getName()), 255, 0, 0)
		end
		...
		self.m_LocationBlips[client]:setDisplayText("Position of "..client:getName())
