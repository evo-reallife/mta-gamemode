...
function GangAreaManager:Event_gangAreaTurfStop(Id, reason, turfingGroupName)
	...
	if reason == TURFING_STOPREASON_LEAVEAREA then
		ShortMessage:new(_"You have left the Gangwar by leaving the area. However, you can re-enter at any time!")
	elseif reason == TURFING_STOPREASON_NEWOWNER then
		ShortMessage:new(_("Gangwar ended! The area now belongs to %s", turfingGroupName))
	elseif reason == TURFING_STOPREASON_DEFENDED then
		ShortMessage:new(_("Gangwar ended! The area was successfully defended from %s", turfingGroupName))
