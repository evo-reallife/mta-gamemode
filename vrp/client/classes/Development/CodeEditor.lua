...
function CodeEditorGUI:constructor()
	...
	self.m_ThemeChanger = GUIGridChanger:new(6, 1, 5, 1, self.m_Window)
	self.m_NameEdit = GUIGridEdit:new(12, 1, 5, 1, self.m_Window)
	self.m_NewBtn = GUIGridIconButton:new(17, 1, FontAwesomeSymbols.File, self.m_Window):setTooltip("New class", "bottom")
	self.m_RefreshBtn = GUIGridIconButton:new(18, 1, FontAwesomeSymbols.Refresh, self.m_Window):setTooltip("Class (new) load", "bottom")
	self.m_CopyBtn = GUIGridIconButton:new(19, 1, FontAwesomeSymbols.Copy, self.m_Window):setTooltip("Copy code", "bottom")
    self.m_EditorBrowser = GUIGridWebView:new(1, 2, 19, 12, "http://mta/local/files/html/editor.htm", true, self.m_Window)
	...
	self.m_NameEdit:setText("Classname")
	...
	self.m_NewBtn.onLeftClick = function()
	self.m_EditorBrowser:callEvent("onCodeEditorCreateClass", "")
end
...
function CodeEditorGUI:onCodeReceive(code)
	if self.m_ReceiveMode == "copy" then
		setClipboard(code)
		InfoBox:new(_"Code saved to the clipboard.")
...
function CodeSession:loadClass(parsedString, name)
	if not self.m_Sessions[name] then
		...
		InfoBox:new(_("Class %s loaded.", name))
		...
		ErrorBox:new(_("Class %s is already loaded", name))
	end
end
