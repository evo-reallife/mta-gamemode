...
function FireManager:checkForFireGroundInfo(uFire)
	if self.m_Fires[uFire] then
		...
		if not isElementStreamedIn(uFire) then return "not streamed" end
		if self.m_Fires[uFire].uEffect:getPosition().z == 0 then return "z auf 0" end
		if self.m_Fires[uFire].uEffect:getPosition().z == self.m_Fires[uFire].baseZ then return "z at base height" end
...
function FireManager:updateStatistics(tblStats, timeSinceStart, timeEstimated, w, h)
	if not self.m_StatisticShortMessage[tblStats.name] then
		self.m_StatisticShortMessage[tblStats.name] = ShortMessage:new("", "Brand overview ("..(tblStats.name)..")", Color.Orange, 6000, nil, function()
			self.m_StatisticShortMessage[tblStats.name] = nil
		end)
	end
	local sm = self.m_StatisticShortMessage[tblStats.name]

	local t = ("Time since outbreak: %s\nFlames: %s active, %s since outbreak\n\nInvolved emergency services:"):format(string.duration((timeSinceStart)/1000), tblStats.firesActive, tblStats.firesTotal)

	for i, v in pairs(tblStats.pointsByPlayer) do
		t = t.. ("\n %s - %s Points (%s fire extinguished)"):format(i:getName(), v, tblStats.firesByPlayer[i] or 0)
	end
	if DEBUG then
		t = t.. ("\n\n~~~DEBUG~~~\nDimension (w,h): %s, %s\nEstimated deletion time (min): %s"):format(w, h, string.duration((timeEstimated)/1000))
	end
	sm:setText(t)
	sm:resetTimeout()
end
...
function FireManager:reloadFiresIfBugged()
	...
	ShortMessage:new(inspect(count), _"updated fire")
end