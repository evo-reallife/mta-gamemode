...
function FactionManager:Event_selfArrestMarker( client )
	if not localPlayer.m_selfArrest then
		...
		QuestionBox:new(
			_"Would you like to turn yourself in with bail?",
			function ()
		...
		if now >= self.m_DrawStart then
			if self.m_SpeedCamSpeed and self.m_SpeedCamVehicle then
				if self.m_bLineChecked == self.m_SpeedCamVehicle then
					...
					local text = ("Radar: %s drives %s km/h in %sem %s!"):format(occ and occ:getName() or "-", speed, colName, vName)
...
function FactionManager:stateFactionOfferTicket( cop )
	ShortMessage:new(_("%s offers you a ticket for the remission of a Wanteds for %s. Click here to accept it!", cop:getName(), toMoneyString(TICKET_PRICE*localPlayer:getWanteds()+500)), "Wanted-Ticket", Color.DarkLightBlue, 30000)
