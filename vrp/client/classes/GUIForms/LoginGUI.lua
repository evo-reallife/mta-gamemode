...
function LoginGUI:loadLoginElements()
	...
	self.m_Elements.lbl1 = GUIGridLabel:new(1, 3, 9, 2, _"Welcome to EVo Reallife, please log in with your account details.", self.m_Window)
		:setAlignX("center")
	...
	self.m_Elements.editPW = GUIGridEdit:new(1, 6, 9, 1, self.m_Window)
		:setCaption(_"Password")
		:setText(self.m_SavedPW or "")
		:setMasked()
		:setIcon(FontAwesomeSymbols.Lock)
	self.m_Elements.lbl2 = GUIGridLabel:new(1, 7, 6, 1, _"Remember password", self.m_Window)
	self.m_Elements.swSavePW = GUIGridSwitch:new(7, 7, 3, 1, self.m_Window):setState(self.m_SavedPW and self.m_SavedPW ~= "")

	self.m_Elements.BtnLogin = GUIGridButton:new(1, 8, 9, 1, "Login", self.m_Window)
		:setBarEnabled(false)
	...
	self.m_Elements.connectionLabel = GUIGridLabel:new(1, 9, 9, 1, "", self.m_Window)
		:setColor(Color.Red)
		:setAlignX("center")
	self.m_Elements.Label = GUIGridLabel:new(1, 10, 9, 1, _"Don't have an account? Register now!", self.m_Window)
		:setClickable(true)
		:setAlignX("center")
...
function LoginGUI:loadRegisterElements()
	...
	self.m_Elements.lbl1 = GUIGridLabel:new(1, 3, 9, 3, _"Please fill out the form to create a new account. Only one account is permitted per PC and Internet connection.", self.m_Window)
		:setAlignX("center")
	...
	self.m_Elements.editPW2 = GUIGridEdit:new(1, 8, 9, 1, self.m_Window)
		:setCaption(_"Confirm Password")
		:setMasked()
		:setIcon(FontAwesomeSymbols.Lock)
	self.m_Elements.editEmail = GUIGridEdit:new(1, 9, 9, 1, self.m_Window)
		:setCaption(_"Email Address")
		:setIcon(FontAwesomeSymbols.Mail)

	self.m_Elements.checkAcceptRules = GUIGridCheckbox:new(1, 10, 7, 1, _"I accept the server rules.", self.m_Window)
	self.m_Elements.cLblRules = GUIGridLabel:new(8, 10, 2, 1, _"(View)", self.m_Window)
		:setClickable(true)
		:setAlignX("right")

	self.m_Elements.BtnRegister = GUIGridButton:new(1, 11, 9, 1, _"Register", self.m_Window)
		:setBarEnabled(false)

	self.m_Elements.Label = GUIGridLabel:new(3, 12, 5, 1, _"Back to login", self.m_Window)
		:setClickable(true)
		:setAlignX("center")
		...
		if self:checkExternalWebsitesActivated() then
			if self.m_Elements.editPW:getText() == self.m_Elements.editPW2:getText() then
				if self.m_Elements.checkAcceptRules:isChecked() then
					...
					triggerEvent("registerfailed",localPlayer, _"You must agree with the server rules!")
				end
			else
				triggerEvent("registerfailed",localPlayer, _"Passwords don't match!")
			end
		end
	end, self)
	...
	-- 	if self:checkExternalWebsitesActivated() then
	-- 		LoginRuleGUI:new()
	-- 		InfoBox:new(_"Since all the rules are in one document, the scroll bar looks surprisingly small - but don't worry, only the rules from §1 - §5 are important for you to start with.\nYou can scroll through the document faster by holding down the Shift key.")
	-- 	end
	-- end
...
function LoginGUI:showRegisterMultiaccountError(name)
	...
	local text = _("An account already exists for your serial. If you want to play with other players in the same network, you must submit a multi account request in the forum (forum.evo-reallife.de).")
	if name then
		text = _("Your serial was last used by player '%s'! If you want to play with other players in the same network, you must submit a multiaccount request in the forum (forum.evo-reallife.de).", name)
	end
...
function LoginGUI:checkExternalWebsitesActivated()
	if getBrowserSettings().RemoteEnabled == false then
		ErrorBox:new(_"Please activate the option Activate external websites in the settings under Web browser! (Re Connect)")
		return false
	end
	return true
end
...
function LoginGUI:setLoggingIn(state)
	...
	if state then
		...
		button:setText(self.m_LoginMode and _"Login" or _"Register")
...
function LoginGUI:updateTexts()
	local button = self.m_LoginMode and self.m_Elements.BtnLogin or self.m_Elements.BtnRegister
	local buttonText = _(self.m_LoginMode and "Login%s" or "Register%s", self.m_Dots)
	local text = ""
	...
	if self.m_QueuePosition and self.m_QueuePosition > 1 then
		text = _("Queue position: %d", self.m_QueuePosition)
	elseif self.m_ConnectionAttempt and self.m_ConnectionAttempt > 0 then
		text = _("Failed to connect to the forum! (%d/%d)\nTry again%s", self.m_ConnectionAttempt-1, FORUM_MAX_CONNECTION_ATTEMPTS-1, self.m_Dots)
	end
	...
	function(queuePosition, connectionAttempt)
		if LoginGUI:isInstantiated() then
			if connectionAttempt == true then
				LoginGUI:getSingleton():setLoggingIn(false)
				ErrorBox:new(_"Connection to forum not possible!")
				return
			end
...
function LoginRuleGUI:constructor()
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Rules and regulations", true, true, self)
