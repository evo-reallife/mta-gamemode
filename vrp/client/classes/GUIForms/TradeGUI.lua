...
function TradeGUI:constructor(target)
    ...
    self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _("Trade with %s", target:getName()), true, true, self)
    ...
    self.m_MyItemsGrid:addColumn(_"Quantity", 0.3)

	self.m_TabWeapons = self.m_TabPanel:addTab(_"Weapons")

    self.m_MyWeaponsGrid = GUIGridList:new(5, 10, 310, 305, self.m_TabWeapons)
    self.m_MyWeaponsGrid:addColumn(_"Weapon", 0.7)
    self.m_MyWeaponsGrid:addColumn(_"Bullets", 0.3)
    ...
    self.m_LabelAmountText = GUILabel:new(340, 200, 110, 30, "Number of items:", self.m_Window)
    ...
    GUILabel:new(340, 240, 220, 30, "money requested:", self.m_Window)
    ...
    self.m_ButtonTrade = GUIButton:new(340, 350, 225, 35, _"Propose trade", self.m_Window):setBackgroundColor(Color.Green):setBarEnabled(true)
	...
	if self.m_Items then
		for index, itemInv in pairs(self.m_Items) do
			if self.m_ItemData then
				if self.m_ItemData[itemInv["Object"]]["Handel"] == 1 then
					item = self.m_MyItemsGrid:addItem(itemInv["Object"], itemInv["Menge"])
					item.onLeftClick = function()
						self.m_SelectedType = "Item"
						self.m_SelectedItem = itemInv["Object"]
						self.m_SelectedItemAmount = itemInv["Quantity"]
						self.m_SelectedItemValue = itemInv["Value"]
						self.m_ButtonTrade:setEnabled(true)
						self.m_Preview:setImage("files/images/Inventory/items/"..self.m_ItemData[itemInv["Object"]]["Icon"])
						self.m_LabelDescription:setText(self.m_ItemData[itemInv["Object"]]["Info"])
						self:checkAmount()
					end
				end
			end
    ...
    if tonumber(amount) then
        if self.m_SelectedItemAmount then
            if tonumber(amount) > self.m_SelectedItemAmount then
                self.m_LabelError:setText(_("You don't have that much %s!", self.m_SelectedItem))
                self.m_ButtonTrade:setEnabled(false)
            else
            ...
            self.m_LabelError:setText(_"Please select an item!")
        end
    else
        self.m_LabelError:setText(_"Please enter a valid quantity!")
    end
end

function TradeGUI:requestTrade()
    if not self.m_SelectedItem then
		ErrorBox:new(_("You have not selected any %s!", self.m_SelectedType))
		return
	end
	...
	if self.m_SelectedType == "Item" then
		if tonumber(self.m_Amount:getText()) > 0 then
			if tonumber(self.m_Amount:getText()) <= self.m_SelectedItemAmount then
				...
				ErrorBox:new(_("You do not have enough %s!", self.m_SelectedItem))
			end
		else
			ErrorBox:new(_"You haven't entered a quantity!")
		end
