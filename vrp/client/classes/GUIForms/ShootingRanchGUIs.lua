...
function ShootingRanch:constructor()
	...
	GUILabel:new(0, 0, self.m_Width, 30, _"Shooting range", self):setAlignX("center"):setAlignY("center"):setColor(Color.Accent)
	self.m_Hits = GUILabel:new(5, 30, self.m_Width-10, 25, _"Please shoot at the targets!", self)
...
function ShootingRanch:updateLabels()
	if getElementData(localPlayer, "ShootingRanch:Data") then
		...
		self.m_Hits:setText(_("Hits: %d/%d", data["Hits"], data["TargetHits"]))
		self.m_TimeLabel:setText(_("Time: %d/%d", self.m_Time, data["Time"]))
		self.m_Accuracy:setText(_("Accuracy: %d/%d", acc, data["TargetAccuracy"]))
...
function ShootingRanchResult:constructor(data, success, totalAmmo)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Shooting range results", true, true, self)
	...
	self.m_Hits:setText(_("Hits: %d needed from %d", data["Hits"], data["TargetHits"]))
	self.m_Time:setText(_("Time needed: %d/%d", time > 60 and 60 or time, data["Time"]))
	self.m_Accuracy:setText(_("Accuracy: %d percent of required %d percent", acc, data["TargetAccuracy"]))
	if success == true then
		GUILabel:new(10, 150, self.m_Width-20, 35, _"Congratulations! You have passed!", self):setColor(Color.Green)
	else
		GUILabel:new(10, 150, self.m_Width-20, 35, _"You have failed!", self):setColor(Color.Red)
	end
end
