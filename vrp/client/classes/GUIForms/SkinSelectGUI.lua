...
function SkinSelectGUI:virtual_destructor()
	...
	if self.m_Edit then
		ErrorBox:new(_"Settings were not saved (please exit edit mode first)!")
	end
end
...
function SkinSelectGUI:setEditable()
	...
	if self.m_Edit then -- perform checks if changes are valid
		...
		if self.m_GroupType == "faction" then
			--if specials < 1 then return ErrorBox:new(_"A maximum of one special clothing may be selected.") end
			if specials > 1 then return ErrorBox:new(_"A maximum of one special clothing may be selected.") end
		end
		if rank0s < 1 then return ErrorBox:new(_"At least one outfit must be set for rank 0.") end
		if self.m_GroupType == "faction" then
