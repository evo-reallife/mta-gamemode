...
function DrivingSchoolInstructorGUI:constructor(type, student)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _("Driving Instructor Menu"), true, false, self)
	GUILabel:new(5, 35, self.m_Width-10, 25, _("Exam: %s", type), self.m_Window)
	GUILabel:new(5, 55, self.m_Width-10, 25, _("Student: %s", student.name), self.m_Window)

	GUILabel:new(5, 75, self.m_Width-10, 25, "Speed:", self.m_Window)
	self.m_SpeedLabel = GUILabel:new(130, 75, self.m_Width-10, 25, "", self.m_Window)

	GUILabel:new(5, 95, self.m_Width-10, 25, "Distance:", self.m_Window)
	self.m_DistanceLabel = GUILabel:new(130, 95, self.m_Width-10, 25, "", self.m_Window)

	GUILabel:new(5, 118, self.m_Width, 30, _("Giving instructions:", student.name), self.m_Window)
	self.m_LeftButton = GUIButton:new(27, 145, 45, 40, "←", self):setBackgroundColor(Color.Accent)
	self.m_LeftButton.onLeftClick = bind(self.turnLeft, self)
	self.m_StraightButton = GUIButton:new(77, 145, 45, 40, "↑", self):setBackgroundColor(Color.Accent)
	self.m_StraightButton.onLeftClick = bind(self.turnStraight, self)
	self.m_RightButton = GUIButton:new(127, 145, 45, 40, "→", self):setBackgroundColor(Color.Accent)
	self.m_RightButton.onLeftClick = bind(self.turnRight, self)
	self.m_TurnButton = GUIButton:new(5, 195, 90, 40, "Turn around ↶", self):setBackgroundColor(Color.Red):setFont(VRPFont(20))
	self.m_TurnButton.onLeftClick = bind(self.turnArround, self)
	self.m_DSButton = GUIButton:new(100, 195, 95, 40, "To The Driving School", self):setBackgroundColor(Color.Orange):setFont(VRPFont(20))
