...
function DrivingSchoolPedGUI:constructor(automaticTestAvailable)
	GUIButtonMenu.constructor(self, "Driving school", 300, 380, false, false, localPlayer.position)
	...
	if automaticTestAvailable then
		self:addItem(_"Annual Audit (Car)", Color.Accent,
			function()
				triggerServerEvent("drivingSchoolStartAutomaticTest", localPlayer, "car")
				self:delete()
			end
		)
		self:addItem(_"Annual Audit (Motorcycle)", Color.Accent,
			function()
		...
		self:addItem(_"Call driving instructor", Color.Accent,
			function()
				self:delete()

				if localPlayer.callInstructorCooldown and getTickCount() - localPlayer.callInstructorCooldown < 60000 then
					ErrorBox:new(_"You've already contacted the driving school!")
					return
				end
	...
	self:addItem(_"Reduce RTR points", Color.Accent,
		function()
			self:delete()
			ReduceSTVOBox:new(player, 1, 20, _"Reduce RTR points", function(category, amount) triggerServerEvent("drivingSchoolReduceSTVO", localPlayer, category, amount) end)
		end
	)
end
...
function ReduceSTVOBox:constructor(player, min, max, title, callback)
	...
	self.m_STVOCategories.onChange = function (text, index)
		...
		self.m_CurrentSTVO:setText(_("Currently %s points", localPlayer:getSTVO(category)))
	end
