...
function MapEditorObjectGUI:constructor(object)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Map Editor: Edit object", true, true, self)
    ...
    self.m_ModelButton = GUIGridButton:new(6, 1, 4, 1, "Change", self.m_Scrollable)
    ...
    self.m_BreakableCheckbox = GUIGridCheckbox:new(1, 11, 10, 1, "Breakable", self.m_Scrollable)
    
    self.m_CollisionCheckbox = GUIGridCheckbox:new(1, 12, 10, 1, "Collisions", self.m_Scrollable)
    
    self.m_DoubleSidedCheckbox = GUIGridCheckbox:new(1, 13, 5, 1, "Double-sided Texture", self.m_Scrollable)
    self.m_TexturesButton = GUIGridButton:new(6, 13, 4, 1, "Textures", self.m_Scrollable)
    
    self.m_LODCheckbox = GUIGridCheckbox:new(1, 14, 10, 1, "Enable LOD (Render building from a distance)", self.m_Scrollable)
    
    self.m_SaveButton = GUIGridButton:new(2, 12, 4, 1, "Save", self.m_Window):setBackgroundColor(Color.Green)
    self.m_DiscardButton = GUIGridButton:new(6, 12, 4, 1, "Discard", self.m_Window):setBackgroundColor(Color.Red)
...
function MapEditorObjectGUI:insertObjectData(object)
    ...
    if object:isBreakable() == false then
        ...
        if object:isBreakable() == false then
            ...
            self.m_BreakableCheckbox:setText("Breakable (Does not affect this object)")
        end
