...
function MapEditorTextureEditGUI:constructor(object, textures)
    ...
    self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Edit textures", true, true, self)
    ...
    self.m_TextureName = GUIGridLabel:new(3, 1, 9, 1, "Texture name: -", self.m_Window)
    self.m_TexturePath = GUIGridLabel:new(3, 2, 9, 1, "Image path: -", self.m_Window)

    self.m_BackgroundCheckbox = GUIGridCheckbox:new(1, 3, 11, 1, "Your image file is in the wrong format! (PNG or JPG only)", self.m_Window)
	...
	self.m_Grid:addColumn("Texture name", 0.4)
	self.m_Grid:addColumn("Image path", 0.6)
    ...
    for textureName, textureMaterial in pairs(objectTextures) do
        ...
        item.onLeftDoubleClick = function()
            ...
            self.m_TextureName:setText(("Texture name: %s"):format(item:getColumnText(1)))
            self.m_TexturePath:setText(("Image path: %s"):format(item:getColumnText(2)))
    ...
    self.m_PathEdit = GUIGridEdit:new(1, 8, 10, 1, self.m_Window):setCaption("Image path")
            ...
            if self:checkForFileEndings(self.m_PathEdit:getText()) then
                ...
                if self:isTextureLocal(self.m_PathEdit:getText()) then
                    if not fileExists(self.m_PathEdit:getText()) or not string.find(self.m_PathEdit:getText(), "files/images/Textures/") then
                        ErrorBox:new("The image file does not exist locally!")
                        return
                    end
                end
                ...
                ErrorBox:new("Your image file is in the wrong format! (PNG or JPG only)")
            end
        end
    end
