...
function MapEditorObjectCreateGUI:constructor(onlyChangeExisting)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, onlyChangeExisting and "Map Editor: Change Model" or "Map Editor: Create Object", true, true, self)
    
    self.m_SearchEdit = GUIGridEdit:new(1, 1, 11, 1, self.m_Window):setCaption("Search")

    self.m_ComboBox = GUIGridCombobox:new(1, 2, 11, 1, "Category Selection", self.m_Window)
	...
	self.m_CreateButton = GUIGridButton:new(1, 26, 5, 1, onlyChangeExisting and "Change" or "Create", self.m_Window):setBackgroundColor(Color.Green)
    self.m_AbortButton = GUIGridButton:new(7, 26, 5, 1, "Cancel", self.m_Window):setBackgroundColor(Color.Red)
