...
function MapEditorMapGUI:constructor()
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Map Editor: Map Administration", true, true, self)
	...
	self.m_EditMap = GUIGridButton:new(1, 11, 4, 1, "Edit", self.m_Window)
	self.m_MapStatus = GUIGridButton:new(5, 11, 4, 1, "Deactivate", self.m_Window)

	self.m_EditingPlayers = GUIGridButton:new(1, 12, 8, 1, "Players Currently Working", self.m_Window)
	
	self.m_Headline = GUIGridLabel:new(9, 1, 11, 1, "Information About Map", self.m_Window):setHeader()

	self.m_EditMapSettings = GUIGridButton:new(16, 1, 4, 1, "Map Settings", self.m_Window)
	
	self.m_NameLabel = GUIGridLabel:new(9, 2, 11, 1, "Name: -", self.m_Window)
	self.m_CreatorLabel = GUIGridLabel:new(9, 3, 11, 1, "Creator: -", self.m_Window)
	self.m_ObjectGrid = GUIGridGridList:new(9, 4, 11, 5, self.m_Window)
	
	self.m_ObjectGrid:addColumn("Type", 0.1)
	self.m_ObjectGrid:addColumn("ID", 0.1)
	self.m_ObjectGrid:addColumn("Name", 0.45)
	self.m_ObjectGrid:addColumn("Creator", 0.35)

	self.m_EditObject = GUIGridButton:new(9, 9, 5, 1, "Edit", self.m_Window):setBackgroundColor(Color.Green)
	self.m_DeleteObject = GUIGridButton:new(15, 9, 5, 1, "Delete", self.m_Window):setBackgroundColor(Color.Red)
	
	self.m_ShowBoundingBox = GUIGridButton:new(9, 10, 5, 1, "Show Frame For All", self.m_Window):setBackgroundColor(Color.Orange)
	self.m_ShowAllObjects = GUIGridButton:new(15, 10, 5, 1, "Show All On Map", self.m_Window):setBackgroundColor(Color.Orange)
	
	self.m_CreateNewLabel = GUIGridLabel:new(9, 11, 5, 1, "Create New Map", self.m_Window):setHeader()
	self.m_NewNameLabel = GUIGridEdit:new(9, 12, 6, 1, self.m_Window):setCaption("Name"):setMaxLength(30)
	self.m_CreateNewButton = GUIGridButton:new(15, 12, 5, 1, "Create Map", self.m_Window):setBackgroundColor(Color.Green)
	...
	self.m_EditMap.onLeftClick = function()
		if localPlayer:getRank() >= ADMIN_RANK_PERMISSION["openMapEditor"] then
			...
			ErrorBox:new("You Are Not Authorized!")
		end
	end
	...
	self.m_ShowBoundingBox.onLeftClick = function()
		...
		InfoBox:new(("The bounding box is now available for all objects on the map %s!"):format(state and "Activate" or "Deactivate"))
	end
	...
	self.m_CreateNewButton.onLeftClick = function()
		if #self.m_NewNameLabel:getText() < 5 then
			ErrorBox:new("Please give the map a reasonable name!")
			return
		end
...
function MapEditorMapGUI:receiveInfos(mapTable)
	...
	for key, infoTable in pairs(mapTable) do
		local item = self.m_GridList:addItem(key, string.short(infoTable[1], 20), infoTable[3] == 1 and "Activate" or "Deactivate")

		item.onLeftClick = function()
			self.m_ObjectGrid:clear()
			self.m_Headline:setText(("Information about Map #%s"):format(key))
			self.m_NameLabel:setText(("Name: %s"):format(infoTable[1]))
			self.m_CreatorLabel:setText(("Creator: %s"):format(infoTable[2]))
			self.m_MapStatus:setText(infoTable[3] == 1 and "Deactivate" or "Activate")
