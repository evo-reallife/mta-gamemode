...
function WorldItemOverviewGUI:constructor(sOwnerName, tblObjects, id, type)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _("Object overview - %s", sOwnerName), true, true, self)
	if type == "player" then
	...
	self.m_ObjectList = GUIGridList:new(5, 65, self.m_Width - 10, 200, self)
		...
		:addColumn(_"Creator", 0.2)
		:addColumn(_"Creation time", 0.3)
	...
	--options
	GUILabel:new(5, 340, self.m_Width, 30, _"Options", self)
	self.m_Changer = GUIChanger:new(5, 375, 250, 30, self)
	self.m_Changer:addItem("Filter (entire list)")
	self.m_Changer:addItem("Selection in list")
	self.m_CollectBtn = GUIButton:new(260, 375, 185, 30, _"Pick up", self):setBarEnabled(true)
	...
	self.m_DeleteBtn = GUIButton:new(450, 375, 185, 30, _"Delete", self):setBackgroundColor(Color.Red):setBarEnabled(true)
...
function WorldItemOverviewGUI:loadObjectsInList(tblObjects)
	...
	self.m_ListSize = i - 1
	self.m_PlacedObjectsLabel:setText(_("placed objects (%d)", self.m_ListSize))
...
function WorldItemOverviewGUI:Event_OnListItemClick()
	nextframe(function()
		...
		if self.m_SelectedListItem and not isElement(self.m_FilteredObjectList[self.m_SelectedListItem.m_Id].Object) then
			WarningBox:new(_"This object no longer exists!")
		end
...
function WorldItemOverviewGUI:Event_OnActionButtonClick(action)
	...
	if type == 1 then -- list
		...
		if action == WorldItemOverviewGUI.Action.Collect then
			QuestionBox:new(_("Do you really want to keep %d objects? (you need the corresponding space in the inventory)", self.m_ListSize),
			function()
		...
		elseif action == WorldItemOverviewGUI.Action.Delete then
			QuestionBox:new(_("Do you really want to permanently delete %d objects?", self.m_ListSize),
			function()
	...
	elseif type == 2 then -- selection
		...
		if self.m_SelectedListItem then
			...
			ErrorBox:new(_"Please select an object from the list or change your selection on the left.")
		end
	end
end
...
addEventHandler("recieveWorldItemListOfOwner", root, function(sOwnerName, tblObjects, id, type)
	if WorldItemOverviewGUI:isInstantiated() then
		if id == WorldItemOverviewGUI:getSingleton().m_OwnerId then
			...
			ErrorBox:new(_"Please close the old object overview window first!")
		end
	else
