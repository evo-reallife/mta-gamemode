...
function SkyscraperGUI:constructor(id, apartments, apartmentsOwner, rangeElement)
    GUIButtonMenu.constructor(self, _("Skyscraper #%s", id), false, false, false, false, rangeElement)
    ...
    for i, houseId in pairs(table.reverse(apartments)) do
        ...
        local floorName = floorNumber == 0 and "Ground Floor" or floorNumber..". Floor"
        local color = apartmentsOwner[i] and Color.Red or Color.Green
        local idPrefix = localPlayer:getRank() >= 4 and "["..houseId.."]" or ""
        local rentStatus = apartmentsOwner[i] and "rented" or "available"

        self:addItem(("%s Apartment %s - %s"):format(idPrefix, floorName, rentStatus), color, bind(self.itemCallback, self, houseId))
    end
end
