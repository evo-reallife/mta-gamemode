...
function StyleguideDemonstrationGUI:constructor()
	...
	self.m_Tabs, self.m_TabPanel = self.m_Window:addTabPanel({"General", "more", "Notes"})
	self.m_TabPanel:updateGrid()

	--Tab 1

	GUIGridLabel:new(1, 1, 5, 1, _"Headline", self.m_Tabs[1]):setHeader()
	GUIGridLabel:new(1, 2, 5, 1, _"Sub-heading", self.m_Tabs[1]):setHeader("sub")
	GUIGridLabel:new(1, 3, 5, 1, _"Infotext / Label", self.m_Tabs[1])


	GUIGridLabel:new(11, 1, 9, 1, "A super great list!", self.m_Tabs[1]):setHeader("sub")
	...
	GUIGridButton:new(7, 2, 4, 1, _"Add", self.m_Tabs[1]):setBackgroundColor(Color.Red)
	GUIGridButton:new(7, 3, 4, 1, _"Remove", self.m_Tabs[1]):setBackgroundColor(Color.Green)
	GUIGridButton:new(7, 4, 4, 1, _"Up", self.m_Tabs[1])
	GUIGridButton:new(7, 5, 4, 1, _"Down", self.m_Tabs[1])
	GUIGridLabel:new(7, 6, 4, 2, _"Multiline\nInfotext\nundso", self.m_Tabs[1])
	...
	GUIGridButton:new(8, 14, 4, 1, "Accept p.", self.m_Tabs[1]):setBarEnabled(false)
	GUIGridButton:new(12, 13, 4, 2, "Take over\nTwo-line", self.m_Tabs[1])
	GUIGridButton:new(16, 14, 4, 1, "Accept", self.m_Tabs[1])

	local prog = GUIGridProgressBar:new(1, 9, 8, 1, self.m_Tabs[1]):setProgress(13.37):setText("for progress!"):setProgressTextEnabled(true)
	prog:setTooltip("this bar will never be full!", "top")
	GUIGridEdit:new(1, 10, 8, 1, self.m_Tabs[1]):setText("These are the new text boxes")
	GUIGridEdit:new(1, 11, 8, 1, self.m_Tabs[1]):setText("with select function!")
	GUIGridLabel:new(1, 12, 4, 1, "Slider from 30 to 40", self.m_Tabs[1])
	...
	GUIGridLabel:new(1, 1, 10, 1, _"any settings", scrollArea):setHeader()
	for i = 1, 5 do
