...
function KartGUI:constructor(marker)
	...
	local tabTimeRace = self.m_TabPanel:addTab(_("Time race"))
	local tabToptimes = self.m_TabPanel:addTab(_("Toptimes"))

	-- Zeitrennen
	GUILabel:new(self.m_Width*0.02, self.m_Height*0.02, self.m_Width*0.4, self.m_Height*0.1, _"EVo Kart Racing", tabTimeRace)

	GUILabel:new(self.m_Width*0.02, self.m_Height*0.11, self.m_Width*0.25, self.m_Height*0.06, _"Current map:", tabTimeRace)
	self.m_MapNameLabel = GUILabel:new(self.m_Width*0.3, self.m_Height*0.11, self.m_Width*0.68, self.m_Height*0.06, "", tabTimeRace)

	GUILabel:new(self.m_Width*0.02, self.m_Height*0.17, self.m_Width*0.25, self.m_Height*0.06, _"Author:", tabTimeRace)
	self.m_AuthorLabel = GUILabel:new(self.m_Width*0.3, self.m_Height*0.17, self.m_Width*0.4, self.m_Height*0.06, "", tabTimeRace)

	GUILabel:new(self.m_Width*0.02, self.m_Height*0.25, self.m_Width*0.8, self.m_Height*0.07, _"Rounds", tabTimeRace)
	...
	GUILabel:new(self.m_Width*0.02, self.m_Height*0.42, self.m_Width*0.25, self.m_Height*0.06, _"Price:", tabTimeRace)
	...
	GUILabel:new(self.m_Width*0.02, self.m_Height*0.5, self.m_Width*0.25, self.m_Height*0.06, _"Required time:", tabTimeRace)
	self.m_CalcedTimeLabel = GUILabel:new(self.m_Width*0.25, self.m_Height*0.5, self.m_Width*0.25, self.m_Height*0.06,"", tabTimeRace)

	GUILabel:new(self.m_Width*0.02, self.m_Height*0.57, self.m_Width*0.98, self.m_Height*0.06, _"ATTENTION: You will not receive a refund if you do not complete all the laps!", tabTimeRace):setColor(Color.Red)

	self.m_GhostCheckbox = GUILabel:new(self.m_Width*0.02, self.m_Height*0.85, self.m_Width, self.m_Height*0.06, _"Double-click on any Toptime to activate the ghost!", tabTimeRace)
	self.m_DisableGhost = GUIButton:new(self.m_Width*0.02, self.m_Height*0.79, self.m_Width*0.3, self.m_Height*0.06, _"Deactivate ghost", tabTimeRace):setBackgroundColor(Color.Red):setVisible(false)
	...
	self.m_GridList:addColumn("Time", .4)
	self.m_GridList:addColumn("Player", .5)
