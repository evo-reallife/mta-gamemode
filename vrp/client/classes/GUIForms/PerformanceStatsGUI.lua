...
function PerformanceStatsGUI:constructor()
	...
	self.m_TabClasslib.m_LogCheckbox = GUICheckbox:new(self.m_Width*0.02, self.m_Height*0.02, self.m_Width*0.3, self.m_Height*0.04, "Start log", self.m_TabClasslib)
	self.m_TabClasslib.m_LogCheckbox.onChange = function(state)
		DEBUG_MONITOR_CLASSLIB = state
	end
	self.m_TabClasslib.m_LogShowButton = GUIButton:new(self.m_Width*0.8, self.m_Width*0.01, self.m_Width*0.18, self.m_Height*0.08-self.m_Width*0.01, "Clear log", self.m_TabClasslib)
	...
	GUILabel:new(self.m_Width*0.35, 0, self.m_Width*0.2, self.m_Height*0.08, "Log out", self.m_TabClasslib)
	...
	self:addField(self.m_TabNetwork, "Monitor warning points", function() return NetworkMonitor:getSingleton():getWarnCount() end)
	...
	if self.m_TabCache.m_Gridlist then
		...
		for path, data in pairs(TextureCache.Map) do
			...
			item.onLeftDoubleClick = function()
				local blips = {}
				local text = _"The following elements use this texture:"
				...
				ShortMessage:new(text, _("Texture Info (%s)", path:gsub("files/images/Textures", "")), Color.Red, -1, function()
	...
	if self.m_TabPerformance.m_Gridlist then
		...
		for i, data in ipairs(f) do
			if data[2] ~= "-" then
				...
				item.onLeftDoubleClick = function()
					setClipboard(data[1])
					ShortMessage:new(data[1].." to the clipboard!")
				end
			end
		end
	end
