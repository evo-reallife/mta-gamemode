...
function PasswordBox:constructor(password, yesCallback, noCallback)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Password", true, true, self)
	GUILabel:new(self.m_Width*0.01, self.m_Height*0.22, self.m_Width*0.98, self.m_Height*0.15, "Please enter your password:", self.m_Window):setFont(VRPFont(self.m_Height*0.17))
	...
	self.m_YesButton = GUIButton:new(self.m_Width*0.1, self.m_Height*0.7, self.m_Width*0.35, self.m_Height*0.2, _"Confirm", self.m_Window):setBackgroundColor(Color.Green)
	self.m_NoButton = GUIButton:new(self.m_Width*0.55, self.m_Height*0.7, self.m_Width*0.35, self.m_Height*0.2, _"Cancel", self.m_Window):setBackgroundColor(Color.Red)

	self.m_YesButton.onLeftClick = function()
		if yesCallback and self.m_PasswordEdit:getText() == password then
			yesCallback()
		else
			ErrorBox:new(_"The password you entered is incorrect!")
		end
