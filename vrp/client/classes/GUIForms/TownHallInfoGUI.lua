...
function TownhallInfoGUI:constructor()
	...
	-- Aktivitäten
	local tabActivities = self.m_TabPanel:addTab(_"Activities")
	...
	-- Inventar/Items
	local tabInventory = self.m_TabPanel:addTab(_"Inventory")
	self.m_TabInventory = tabInventory

	-- Info
	local tabInfo = self.m_TabPanel:addTab(_"Info")
	self.m_TabInfo = tabInfo
	GUILabel:new(self.m_Width*0.02, self.m_Height*0.02, self.m_Width*0.31, self.m_Height*0.10, _"Information", tabInfo)
