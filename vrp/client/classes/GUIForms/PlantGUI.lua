...
function PlantGUI:constructor(id, type, lastGrow, size, maxSize, item, itemsPerSize, owner, lastWatered, wateredTime)
	...
	GUILabel:new(10, 40, self.m_Width-20, 25, _("Size: %d/%d", size, maxSize), self):setAlignX("center")
	if size < maxSize then
		local nextGrow = lastGrow+60*60
		nextGrow = math.floor((nextGrow-ts)/60)
		GUILabel:new(10, 70, self.m_Width-20, 20, _("Next growth in ~%d minutes", nextGrow), self):setAlignX("center")
	end
	GUILabel:new(10, 90, self.m_Width-20, 20, _("Current harvest: %d/%d %s", size*itemsPerSize, maxSize*itemsPerSize, item), self):setAlignX("center")
	local watered = math.floor(lastWatered+wateredTime*60*60-ts)
	if watered > 0 then
		local wateredOptical = timespanArray(watered)
		GUILabel:new(10, 110, self.m_Width-20, 20, _("Watered for %s hours", wateredOptical["hour"]..":"..wateredOptical["min"]), self):setAlignX("center")
	else
		GUILabel:new(10, 110, self.m_Width-20, 20, _"not watered", self):setColor(Color.Red):setAlignX("center")
	end
	GUILabel:new(10, 130, self.m_Width-20, 20, _"Press [E] to harvest!", self):setColor(Color.Accent):setAlignX("center")
