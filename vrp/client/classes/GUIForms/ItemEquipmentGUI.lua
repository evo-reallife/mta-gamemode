...
function ItemEquipmentGUI:constructor(id, element)
	...
	self.m_Window = GUIWindow:new(0, 0, self.m_Width, self.m_Height, _"Equipment storage", true, true, self)
	GUIGridRectangle:new(10, 2, 5, 1, Color.Grey, self.m_Window)

	self.m_GridInventar = GUIGridGridList:new(1, 2, 9, 8, self.m_Window)
	self.m_GridInventar:addColumn(_"Equipment", 0.7)
	self.m_GridInventar:addColumn(_"item", 0.3)
	GUIGridRectangle:new(1, 1, 9, 1, Color.Grey, self.m_Window)
	GUIGridLabel:new(1, 1, 9, 1, "Inventory", self.m_Window):setAlignX("center")

	self.m_GridDepot = GUIGridGridList:new(11, 2, 10, 8, self.m_Window)
	self.m_GridDepot:addColumn(_"Equipment", 0.7)
	self.m_GridDepot:addColumn(_"item", 0.3)
	GUIGridRectangle:new(11, 1, 10, 1, Color.Grey, self.m_Window)
	GUIGridLabel:new(11, 1, 10, 1, "Storage", self.m_Window):setAlignX("center")
...
function ItemEquipmentGUI:isSpecialProduct(product)
	return (product == "RPG-7" or product == "Grenade" or product == "Sniper rifle" or product == "Gas grenade")
end
...
function ItemEquipmentGUI:loadInventoryItems()
    ...
    for index, itemInv in pairs(self.m_Items) do
        if self.m_ItemData[itemInv["Object"]]["Trade"] == 1 then
            if self.m_AllowedItems["Special"][itemInv["Object"]] or self.m_AllowedItems["Explosive"][itemInv["Object"]] then
                item = self.m_GridInventar:addItem(itemInv["Objekt"], itemInv["Quantity"])
                item.onLeftClick = function()
                    self.m_SelectedItemInventar = itemInv["Object"]
                    self.m_SelectedItemAmountInventar = itemInv["Quantity"]
                    self:updateInfo(self.m_ItemData[itemInv["Object"]]["Icon"], self.m_ItemData[itemInv["Object"]]["Info"])
                end
            end
        end
    end
