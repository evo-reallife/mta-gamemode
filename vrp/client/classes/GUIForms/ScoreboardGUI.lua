...
function ScoreboardGUI:constructor()
	...
	self.m_Logo = GUIImage:new(self.m_Width-self.m_Width*0.18, self.m_Height*0.83, self.m_Width*0.177, self.m_Width*0.075, "files/images/LogoNoFont.png", self)
	...
	self.m_Grid:addColumn(_"Name", 0.21)
	self.m_Grid:addColumn(_"Faction", 0.1)
	self.m_Grid:addColumn("", 0.04)
	self.m_Grid:addColumn(_"Company", 0.15)
	self.m_Grid:addColumn(_"Gang/Corporation", 0.23)
	self.m_Grid:addColumn(_"Playtime", 0.12)
	self.m_Grid:addColumn(_"Ping", 0.07)
	self.m_Grid:setSortable{_"VIP", _"Name", _"Faction", _"Company", _"Gang/Firm", _"Playtime"} --We can't sort Ping (Ping can be a number and also a string)
...
function ScoreboardGUI:refresh()
	...
	self:addPlayerCount(_"Civilians", self.m_FactionCount[0] or 0, self.m_FactionAFKCount[0] or 0, tocolor(255, 255, 255))
	...
	self.m_PlayerCount:setText(_("There are currently %d players online", #getElementsByType("player")))
	self.m_Ping:setText(_("Ping: %dms", localPlayer:getPing()))
end
...
function ScoreboardGUI:insertPlayers()
	for index, playerTable in ipairs(self.m_Players) do
		...
		local item = self.m_Grid:addItem(
			...
			isLoggedIn and (player:getFaction() and not player:getFaction():isInsurgentFaction() and player:getFaction():getShortName() or "- None -") or "-", -- not = Dont show player as Insurgent (Insurgent = Civilian)
			...
			isLoggedIn and (player:getCompany() and player:getCompany():getShortName()  or "- None -") or "-",
