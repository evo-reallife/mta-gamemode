...
function Tour:constructor()
  ...
  if core:get("Tour", "done", false) == false then
    QuestionBox:new(
      _("Would you like to start a server tour? Upon completion you will receive experience and a small reward! (If the mouse pointer is not active, press 'B')"),
      function() triggerServerEvent("tourStart", localPlayer) end, function() InfoBox:new("You can restart the tour at any time under F2 -> Settings!") core:set("Tour", "done", true) end)
  end
...
function Tour:show(id, title, description, success, x, y, z)
  ...
  self.m_TargetBlip:setDisplayText("next tour destination")
