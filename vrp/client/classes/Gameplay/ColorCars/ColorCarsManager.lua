...
function ColorCarsManager:refreshLobbyGUI()
    ...
    for i = #self.m_LobbyInfos, 1, -1 do
        ...
        local ColorCarsLobby = ColorCarsLobbyGUI:getSingleton().m_LobbyGridList:addItem(v["LobbyName"], ("%s / %s"):format(v["Players"], v["maxPlayers"]), v["hasPassword"] and "Yes" or "No")
...
function ColorCarsManager:Event_receiveMaxPlayersResult(isFull)
    if not isFull then 
        ...
        ErrorBox:new(_"The lobby is full.")
    end
end
