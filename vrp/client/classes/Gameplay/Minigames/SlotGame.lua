...
SlotGame.HelpText = "Spin with the >>Play<< button \nYour win is shown at the top in the >Win< field.\nYour current bet in the >Credits< field.\nYou can increase/decrease this with the >Bet up< and >Bet down< buttons. \nYellow line = 1x win | Red line = 2x win\nUpper green = 3x win | Lower green = 4x win | Middle green = 5x win\nSymbols = Alarm < Hook < Flower < Ghost < Dice < Turtle (< means less value)"
SlotGame.BetAmount = 
{
	[1] = 50, 
	[2] = 100, 
...
function SlotGame:showWin() 
	...
	if self.m_LastPay > 0 then
		...
		outputChatBox(_("#FFFF00[slot machine hall]#FFFFFF You have #00FF00$%s#FFFFFF won!", self.m_LastPay), 255, 255, 255, true)
	else 
