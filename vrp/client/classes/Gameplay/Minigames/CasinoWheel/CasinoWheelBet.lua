...
function CasinoWheelBet:createFields() 
    ...
    for field, label in pairs(self.m_Fields) do 
        label.onHover = function()
            nextframe(function()
                self.m_InfoLabel:setText(("%s - You have set the %s field!"):format(label.value, toMoneyString(self:calcBetOnField(field))))
            end)
        end
...
function CasinoWheelBet:placeToken(field)
    ...
    self:updateBetGui()
    self.m_InfoLabel:setText(("%s - You have set the %s field!"):format(field, toMoneyString(self:calcBetOnField(field))))
...
function CasinoWheelBet:removeTokensFromField(field, output)
    ...
    if self.m_PlacedToken[field] then
        ...
        self:updateBetGui()
        if output then ShortMessage:new(_("You removed all tokens from the %s field!", field)) end
    end
end

function CasinoWheelBet:updateBetGui()
    self.m_BetLabel:setText(("Total bet: %s"):format(toMoneyString(self:calcBet())))
end
...
function CasinoWheelBet:createTokens()
    ...
    for color, amount in spairs(ROULETTE_TOKENS, function(t,a,b) return t[b] > t[a] end) do
        ...
        self.m_Token[color].onHover = function() self.m_InfoLabel:setText(("%ser Jetton - Value per piece: %s"):format(CasinoWheelBet.ColorNames[color], toMoneyString(amount))) end
