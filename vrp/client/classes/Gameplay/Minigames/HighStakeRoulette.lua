HighStakeRoulette = inherit(GUIForm)
inherit(Singleton, HighStakeRoulette)

HighStakeRoulette.ColorNames = {
    ["blue"] = "blue",
    ["green"] = "green",
    ["pink"] = "pink",
    ["red"] = "red",
...
function HighStakeRoulette:constructor()
    ShortMessage:new((("The maximum stake is $%s"):format(convertNumber(getElementData(root, "HighStakeMaxBet") or "500000"))), "High-Stake Roulette", Color.Orange)
    GUIForm.constructor(self, screenWidth/2 - 1024/2, screenHeight/2-506/2, 1024, 506, false)
    self.m_CurrentNumber = 0

	self.m_CircleRot = 0
	self.m_CircleAnimation = CAnimation:new(self, bind(HighStakeRoulette.onSpinDone, self), "m_CircleRot")
    ...
    addEventHandler("highStakeRouletteStartSpin", root, self.m_StartSpinEvent)
    addEventHandler("highStakeRouletteRemoveTableLock", root, self.m_TableLockBind)
end

function HighStakeRoulette:destructor()
    GUIForm.destructor(self)
    removeEventHandler("highStakeRouletteStartSpin", root, self.m_StartSpinEvent)
    removeEventHandler("highStakeRouletteRemoveTableLock", root, self.m_TableLockBind)
end

function HighStakeRoulette:loadColors()
    self.m_Color = {}
...
function HighStakeRoulette:updateRenderTarget()
...
function HighStakeRoulette:onSpinDone()
    triggerServerEvent("highStakeRouletteOnSpinDone", localPlayer, clientNumber)
	setTimer(function()
		 self.m_TableLocked = false
		 self:clearTokens()
	end, 2000, 1)
...
function HighStakeRoulette:spin()
    if self.m_TableLocked then
        ErrorBox:new(_"The table is reserved!")
        return
    end
	if self:calcBet() == 0 then
		ErrorBox:new(_"You haven't placed anything!")
        return
	end
	if localPlayer:getMoney() < self:calcBet() then
		ErrorBox:new(_"You don't have enough money!")
        return
	end
	self.m_TableLocked = true
    triggerServerEvent("highStakeRouletteSpin", root, self.m_PlacedToken)
end

function HighStakeRoulette:cheatSpin(value)
    if self.m_TableLocked then
        ErrorBox:new(_"The table is reserved!")
        return
    end
	self.m_TableLocked = true
    triggerServerEvent("highStakeRouletteCheatSpin", root, self.m_PlacedToken, value)
end


function HighStakeRoulette:Event_startSpin(target)
...
function HighStakeRoulette:createFields()
    self.m_Fields = {}
	...
	for field, fieldElement in pairs(self.m_Fields) do
        fieldElement.onHover = function()
            nextframe(function()
                ...
                self.m_InfoLabel:setText(("Field \"%s\" - You have set to the field %s!"):format(field, toMoneyString(self:calcBetOnField(field))))
            end)
        end
...
function HighStakeRoulette:placeToken(field)
    if self.m_TableLocked then
        ErrorBox:new(_"The table is reserved!")
        return
    end
    ...
    if self:calcBet() + ROULETTE_TOKENS[color] > (getElementData(root, "HighStakeMaxBet") or 500000) then
        ErrorBox:new(_("The maximum bet is %s!", convertNumber(250000)))
        return
    end
    ...
    self:updateBetGui()
    self.m_InfoLabel:setText(("Field \"%s\" - You have set to the field %s!"):format(field, toMoneyString(self:calcBetOnField(field))))
    ShortMessage:new(_("You have set your %d. %sen token to the %s field!", self.m_PlacedToken[field][color], Roulette.ColorNames[color], field), "Roulette")
    setTimer(function()
        self:attachTokenToMouse(color)
    end, 75, 1)
end

function HighStakeRoulette:clearTokens(output)
    if self.m_TableLocked then
		ErrorBox:new(_"The table is reserved!")
        return
    end
    ...
    self:updateBetGui()
	if output then
		ShortMessage:new(_"You have removed all chips from the table!")
	end
end

function HighStakeRoulette:removeTokensFromField(field, output)
    if self.m_TableLocked then
        ErrorBox:new(_"The table is reserved!")
        return
    end
    ...
    if self.m_PlacedToken[field] then
        ...
        self:updateBetGui()
        if output then ShortMessage:new(_("You have removed all tokens from the %s field!", field)) end
    end
end

function HighStakeRoulette:updateBetGui()
    HighStakeRouletteGUI:getSingleton().m_BetLabel:setText(("Total bet:\n%s"):format(toMoneyString(self:calcBet())))
end
...
function HighStakeRoulette:getDifferentTokensOnField(field)
    local count = 0
    if self.m_PlacedTokenImages[field] then
...
function HighStakeRoulette:attachTokenToMouse(color)
...
function HighStakeRoulette:renderMouseToken()
...
function HighStakeRoulette:createTokens()
    self.m_Token = {}
    local i = 0
    ...
    for color, amount in spairs(ROULETTE_TOKENS, function(t,a,b) return t[b] > t[a] end) do
        ...
        self.m_Token[color].onHover = function() self.m_InfoLabel:setText(("%ser Jetton - value per piece: %s"):format(HighStakeRoulette.ColorNames[color], toMoneyString(amount))) end
...
function HighStakeRoulette:removeTableLock()
    self.m_TableLocked = false
end

addEventHandler("highStakeRouletteOpen", root, function()
    HighStakeRoulette:new()
end)


addEventHandler("highStakeRouletteClose", root, function()
    delete(HighStakeRoulette:getSingleton())
end)
