...
function TurtleRace.load()
	...
	ped:setFrozen(true)
	ped.SpeakBubble = SpeakBubble3D:new(ped, "Toad professional Diana", "TURTLE RACE!")
...
addEventHandler("turtleRaceCountdown", root,
	function()
		...
		local shortMessage = ShortMessage:new(_"The turtle race starts in 3 seconds!", _"Turtle race", {50, 170, 20}, 3000)
		local countdown = 3
		setTimer(
			function()
				countdown = countdown - 1
				shortMessage:setText(_("The turtle race starts in %s seconds!", countdown))
			end, 1000, 3
		)
	end
)
