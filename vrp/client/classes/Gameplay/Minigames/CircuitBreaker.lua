...
function CircuitBreaker:constructor(callbackEvent, callbackEventStepFailed, element)
	...
	self.m_RangeColShape = createColSphere(element:getPosition(), 5)
	self.m_RangeColShape:attach(element)
...
function CircuitBreaker:onClientRender()
	...
	dxDrawText("ENTER = START       SPACE = BACK",(screenWidth/2 - self.WIDTH/4), (screenHeight*0.6 - self.HEIGHT/2)+self.HEIGHT, (screenWidth/2 - self.WIDTH/4)+self.WIDTH, screenHeight, tocolor(255,255,255,255),2,"default-bold","center","top")
...
function CircuitBreaker:createStructurGroup(width, height, count)
	...
	if count > 3 then
		return outputConsole("Too little video memory in MTA memory!")
	end
