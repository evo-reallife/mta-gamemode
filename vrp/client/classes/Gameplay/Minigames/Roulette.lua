...
function Roulette:spin()
	...
	if self:calcBet() == 0 then
		ErrorBox:new(_"You haven't set anything!")
        return
	end
...
function Roulette:placeToken(field)
    ...
    if self:calcBet() + ROULETTE_TOKENS[color] > ROULETTE_MAX_BET and not (Sewers:getSingleton().m_AppliedCasino) then
        ErrorBox:new(_("The maximum bet is %s!", toMoneyString(ROULETTE_MAX_BET)))
        return
    end
...
function Roulette:clearTokens(output)
    ...
    self:updateBetGui()
	if output then
		ShortMessage:new(_"You have removed all the chips from the table!")
	end
end
...
function Roulette:removeTokensFromField(field, output)
    ...
    if self.m_PlacedToken[field] then
        ...
        self:updateBetGui()
        if output then ShortMessage:new(_("You have removed all chips from the %s field!", field)) end
    end
end
...
function Roulette:createTokens()
    ...
    for color, amount in spairs(ROULETTE_TOKENS, function(t,a,b) return t[b] > t[a] end) do
        ...
        self.m_Token[color].onHover = function() self.m_InfoLabel:setText(("%ser Jetton - value per piece: %s"):format(Roulette.ColorNames[color], toMoneyString(amount))) end
