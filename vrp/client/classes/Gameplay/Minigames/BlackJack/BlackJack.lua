...
function BlackJack:setup() 
	...
	if not self.m_Spectate then
		GUILabel:new(self.m_Width/2-36, self.m_Height/2+70, 72, 10, "You", self):setFont(VRPFont(20)):setAlignX("center")
	else 	
	...
	self.m_RestartButton = GUIButton:new(self.m_Width/2-300, 506-120, 160, 30, "Again", self):setAlternativeColor(Color.Clear):setBackgroundColor(Color.White):setEnabled(false):setVisible(false)
	...
	self.m_InfoButton = GUIButton:new(self.m_Width-170, 20, 160, 30, "Help", self):setAlternativeColor(Color.Clear):setBackgroundColor(Color.White)
	...
	self.m_InsuranceLabel = GUILabel:new(self.m_Width/2-300, 506-130, 160, 30, "Insurance bet active!", self):setFont(VRPFont(20)):setAlignX("center"):setAlignY("center"):setVisible(false)
	...
	self.m_SpectatorGrid:addColumn("Spectators 0", 1)
end
...
function BlackJack:Event_UpdateSpectator(spectators) 
	...
	self.m_SpectatorGrid:setColumnText(1, ("Spectators %s"):format(count))
end
...
function BlackJack:onRender() 
	...
	if not self.m_Specting and self.m_Object and isElement(self.m_Object) then 
		...
		if getDistanceBetweenPoints3D(x, y, z, x2, y2, z2) > 5 then 
			self:cancel()
			ErrorBox:new(_"You're too far away from blackjack!")
		end
	end
