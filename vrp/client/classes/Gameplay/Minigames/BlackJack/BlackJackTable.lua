			...
			if dist < 10 and isLineOfSightClear(lx, ly, lz, x, y, z, true, false, false, true, false, false, false, localPlayer) then
				...
				if isElementOnScreen(obj) and sx and sy then
					if obj:getData("BlackJack:TableBet") then 
						...
						dxDrawText("Right-click to watch", (sx-tw*0.5)+1, (sy+th*1.1)+1, sx+tw*0.5, sy+th*1.1, Color.Black, 0.9 * distModifier, "sans", "center")
						dxDrawText("Right-click to watch", sx-tw*0.5, sy+th*1.1, sx+tw*0.5, sy+th*1.1, Color.White, 0.9 * distModifier, "sans", "center")
					else 
						local tw = dxGetTextWidth("Table available", 1.4, "sans") * distModifier
						dxDrawBoxShape((sx-tw*0.55)+1, (sy-th*0.05)+1, tw*1.1, th*1.1, Color.Black)
						dxDrawBoxShape(sx-tw*0.55, sy-th*0.05, tw*1.1, th*1.1)
