...
function MechanicTow:constructor()
	...
	self.m_Ped:setData("onClickEvent",
		function()
			self.ms_SelectionGUI = GUIButtonMenu:new("Vehicle type")
			self.ms_SelectionGUI:addItem(_"Private vehicle", Color.Accent,
				function()
					triggerServerEvent("mechanicOpenTakeGUI", localPlayer, "permanentVehicle")
				end
			)
			self.ms_SelectionGUI:addItem(_"Company/Group Vehicle", Color.Accent,
				function()
	...
	SpeakBubble3D:new(self.m_Ped, _"Buy vehicle free", _"Click me!")
	NonCollisionArea:new("Cuboid", {Vector3(2425.22, -2143.81, 12), 23, 18, 5})

	self.m_BugPed = createPed(50, 2433.48, -2102.80, 13.55, 180)
	...
	SpeakBubble3D:new(self.m_BugPed, _"I can detect bugs", _"Click me!")
...
function MechanicTow:requestFill(vehicle, fuel)
	if not vehicle.controller then
		ErrorBox:new(_"There is no player in the vehicle")
		return
	end
