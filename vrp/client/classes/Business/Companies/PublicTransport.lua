...
function PublicTransport:constructor()
	...
	SpeakBubble3D:new(self.m_Ped, _"Vehicle importer", _"Start the vehicle transport")
...
function PublicTransport:onImportListRequest()
	if not localPlayer:getCompany() or localPlayer:getCompany():getId() ~= CompanyStaticId.EPT then 
		ErrorBox:new(_"Only members of the EPT can import vehicles.")
		return
	end
	if not localPlayer:getPublicSync("Company:Duty") then
		ErrorBox:new(_"You are not in the company service.")
		return
	end
	if not PermissionsManager:getSingleton():hasPlayerPermissionsTo("company", "startVehicleImport") then
		ErrorBox:new(_"You are not authorized to start a vehicle import!")
		return
	end
...
function PublicTransport:setBusDisplayText(vehicle, text, line)
	...
	if line then
		dxDrawText("Line "..line, 10, 110, 246, 140, Color.Yellow, 1, getVRPFont(self.m_Font), "left", "center", false, true)
	end
...
function PublicTransport:busStopStreamIn(obj)
	...
	if not source.m_BusCol then
		...
		addEventHandler("onClientColShapeHit", source.m_BusCol, function(hit, dim)
			...
			InfoBox:new(_"Click on the bus stop to see the bus timetable.")
		end)
	end
end
	...
	if getPedOccupiedVehicle(localPlayer) == vehicle and line then
		...
		for stopObj, name in pairs(self.m_StreamedInBusObjects) do -- output message at next stop
			if name == nextStopName then
				ShortMessage:new(_("A bus on line %d (%s) will arrive shortly at the bus stop %s.", line, EPTBusData.lineData.lineDisplayData[line].displayName, nextStopName), _("Public Transport"), {230, 170, 0})
	...
	if EPTBusData.lineData then -- check if line data is instantiated
		...
		if self.m_ActiveBusVehicles[vehicle] and not line then -- vehicle offduty
			...
			if self.m_ActiveLines[line] == 0 then
				ShortMessage:new(_("Bus line %d (%s) is unfortunately no longer in service.", line, EPTBusData.lineData.lineDisplayData[line].displayName), _("Public Transport"), {230, 170, 0})
			end
		...
		elseif not self.m_ActiveBusVehicles[vehicle] and line then -- vehicle onduty
			...
			if self.m_ActiveLines[line] == 0 then
				ShortMessage:new(_("Bus line %d (%s) is in service again.", line, EPTBusData.lineData.lineDisplayData[line].displayName), _("Public Transport"), {230, 170, 0})
			end
			self.m_ActiveLines[line] = self.m_ActiveLines[line] + 1
		end
	end
...
function BusLineMouseMenu:constructor(posX, posY, element)
	GUIMouseMenu.constructor(self, posX, posY, 300, 1)
	 ...
	 self:addItem(_"Change scoreboard"):setTextColor(Color.Red)

	self:addItem(_"Operate line 1 to Blueberry",
		function()
	...
	self:addItem(_"Operate line 1 to Ganton",
		function()
			if self:getElement() then
	...
	self:addItem(_"Operate line 2 to Dillimore",
		function()
			if self:getElement() then
	...
	self:addItem(_"Operate line 2 to Jefferson",
		function()
			if self:getElement() then
