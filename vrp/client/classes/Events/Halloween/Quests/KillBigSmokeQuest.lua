...
function KillBigSmokeQuest:startQuest()
    self:createDialog(bind(self.onStart, self), 
        "You have to find this spirit!",
        "Here, the spirit expeller!"
    )
end

function KillBigSmokeQuest:onStart()
    triggerServerEvent("Halloween:giveGhostCleaner", localPlayer)
    self.m_QuestMessage = ShortMessage:new("Find the hiding place of the ghost!", "Halloween: Quest", Color.Orange, -1, false, false, false, false, true)
end
...
function KillBigSmokeQuest:onGhostKill()
    ...
    sound:setVolume(2)
    sound:setDimension(localPlayer:getDimension())
