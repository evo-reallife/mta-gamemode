...
function GhostKillerQuest:startQuest()
    self:createDialog(bind(self.onStart, self), 
        "You again!",
        "Apparently it's not just my house that's haunted...",
        "Will you do me a favor and take a look around?"
    )
end

function GhostKillerQuest:onStart()
    triggerServerEvent("Halloween:giveGhostCleaner", localPlayer)
    self.m_QuestMessage = ShortMessage:new("Drive the ghosts out of the houses in Los Santos!\n(4 remaining)", "Halloween: Quest", Color.Orange, -1, false, false, false, false, true)
end
...
function GhostKillerQuest:onGhostKill()
    if self.m_KilledGhosts == 3 then
        ...
        self.m_QuestMessage:setText(("Drive the ghosts out of the houses in Los Santos!\n(%s remaining)"):format(4-self.m_KilledGhosts))
    end
end
