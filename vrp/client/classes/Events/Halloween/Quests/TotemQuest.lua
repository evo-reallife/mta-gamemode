...
function TotemQuest:startQuest()
    self:createDialog(bind(self.onStart, self), 
        "So you say totems...",
        "You should have a look around to see if you can find one of these totems!"
    )
end

function TotemQuest:onStart()
    triggerServerEvent("Halloween:giveGhostCleaner", localPlayer)
    self.m_QuestMessage = ShortMessage:new("Find a totem!", "Halloween: Quest", Color.Orange, -1, false, false, false, false, true)
end
    ...
    if matchingDimension then
        if hitElement == localPlayer then
            self:createDialog(bind(self.startTotemHunt, self), 
                "A giant totem spits out ghosts...",
                "You feel a strange presence...",
                "You should not get any closer to the totem..."
            )
...
function TotemQuest:endQuest()
    self:createDialog(false, 
        "A huge totem from which ghosts appear?",
        "And many more small totems around it?",
        "And the great totem has a strange presence around it?",
