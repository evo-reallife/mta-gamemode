...
function AdminEvent:sendAuctionData(data)
    ...
    if data then
        if not self.m_AuctionMessage then
            self.m_AuctionMessage = ShortMessage:new("", "Auction summary", Color.Accent, -1)
        end
        local bids = ""
        for i = 1, (math.min(#data.bids, 5)) do
            local v = data.bids[i]
            bids = bids .. "\n" .. toMoneyString(v[2]) .. " from " .. v[1]
        end
        self.m_AuctionMessage:setText(("It will be %s auctioned, you can bid with /bid Place a bid for [amount].\nCurrent bids:%s"):format(data.name, bids))
